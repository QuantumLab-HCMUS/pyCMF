...
import Molecule_Input_ccpvtz as Molecule_Input
...
def Plot_Data(R):
    ...
    for j in range(mol.nao_nr()):
        print(f"\n===== MO {j} =====  energy: {robmp.mo_energy[j]:12.6f} ===== normalization: {robmp.mo_coeff[:,j].T @ S @ robmp.mo_coeff[:,j]}" )
        prob = (S12 @ robmp.mo_coeff[:,j])**2
        prob /= prob.sum()
        print(f"orth: {sum(prob)}")
        C = robmp.mo_coeff[:,j]
        contrbS = C*(S@C)
        print(f"norm: {sum(contrbS)}")
        for ao, coeff in enumerate(robmp.mo_coeff[:,j]):
            if abs(coeff) < 1e-15:
                coeff = 0
            else: coeff
            if prob[ao] > cutoff:
    ...
    return e_rhf, e_casci, e_obmp2, e_dfold_fci, e_sqd, e_dfold_sqd, e_ccsdt
