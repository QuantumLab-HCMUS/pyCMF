...
h1_R_trunc, h1_R_trunc_dict = truncate_h_real_space(h1_wann_R, R_cut=R_cut)
...
def build_h1_big(h1_R, cluster_R, n_wann, tol=1e-12):
    ...
    for i, Ri in enumerate(cluster_R):
        for j, Rj in enumerate(cluster_R):
            # deltaR = Ri - Rj
            dR = (Ri[0] - Rj[0], Ri[1] - Rj[1], Ri[2] - Rj[2])
...
h2_R = np.fft.fftn(h2_wann_k[..., :, :, :, :], axes=(0, 1, 2)) / (Nkx * Nky * Nkz)
