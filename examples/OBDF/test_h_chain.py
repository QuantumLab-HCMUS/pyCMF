...
def patched_make_veff_core(mp):
    ...
    if ncore_total == 0:
        # Lấy số lượng k-points từ shape của mo_energy
        nkpts = np.shape(mp.mo_energy)[0]
        # Trả về list gồm các số 0.0 với số lượng tương ứng số k-points
        return [0.0] * nkpts
...
def run_scan_and_plot():
    # 1. Định nghĩa dải R cần quét (khớp với các điểm trên trục x của Hình 2 và 3)
    # Các điểm phổ biến trong bài báo: 1.4, 1.8, 2.0, 2.4, 2.8, 3.2, 3.6
    R_list = np.array([1.4, 1.8, 2.0, 2.4, 2.8, 3.2, 3.6])

    # Số k-points để test nhanh (hãy đổi thành 89-229 nếu muốn kết quả giống hệt TDL)
    nk_x_test = 8

    # Các mảng lưu trữ kết quả
    energies = []
    occ_orb1 = []
    occ_orb2 = []

    print('\n' + '#' * 60)
    print('BẮT ĐẦU QUÉT TỰ ĐỘNG ĐỒ THỊ 1D HYDROGEN SOLID')
    print('#' * 60)
    ...
    for R in R_list:
        ...
        try:
            # Gọi hàm tính toán
            E_tot, occupations = run_1d_hydrogen_point(R, nk_x=nk_x_test)
            
            energies.append(E_tot)
            # Sắp xếp occupation để đường đồ thị không bị chéo nhau (orbital bị lật)
            # Thông thường 1 orbital sẽ đầy (gần 2) và 1 trống (gần 0), 
            # giãn R ra thì cả 2 tiến về 1.
            occ_sorted = np.sort(occupations) 
            occ_orb1.append(occ_sorted[0]) # Orbital có occ thấp (Unoccupied)
            occ_orb2.append(occ_sorted[1]) # Orbital có occ cao (Occupied)
            
        except Exception as e:
            print(f"[!] Lỗi tại R = {R}: {e}")
    ...
    # ---- Đồ thị 1: Total Energy (Giống Hình 2) ----
    ax1.plot(R_list, energies, marker='s', linestyle='-', color='teal', label=f'SEET(FCI) Downfold - nk={nk_x_test}')
    ...
    # ---- Đồ thị 2: Occupation Numbers (Giống Hình 3) ----
    ...
    # Đường gióng minh họa giới hạn Strongly Correlated (cả 2 tiến về 1)
