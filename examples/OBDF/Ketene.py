...
# from mp import UOBMP2, UOBMP2_downfold
...
def print_ref(ci_vecs, norbcas, neleccas, nocc_inact, ene, s2_casci, m_casci, ene_casci):
    ...
    for i, ci in enumerate(ci_vecs):
        ...
        print(
            f'<S^2>_casci = {s2_casci[i]:.0f}, multiplicity_casci = {m_casci[i]:.0f}',
            'E_excited_casci:',
            ene_casci[i] - ene_casci[0],
        )
...
mol.atom = [
    ['C', (0.0, 0.0, -1.29547953)],
    ['C', (0.0, 0.0, 0.01851350)],
    ['O', (0.0, 0.0, 1.18357846)],
    ['H', (0.0, 0.93893013, -1.81881376)],
    ['H', (0.0, -0.93893013, -1.81881376)],
]

"""
mol.atom = [
    ['C', ( 1.08114240, 0.0,-1.29547953)],
    ['C', ( 0.00212157, 0.0, 0.01851350)],
    ['0', (-1.18382434, 0.0, 1.18357846)],    
    ['H', ( 0.86575253, 0.0,-1.81881376)],
    ['H', ( 2.09666784, 0.0,-1.81881376)]
]
