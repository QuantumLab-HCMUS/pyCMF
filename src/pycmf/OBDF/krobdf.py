...
def kernel(mp, mo_energy, mo_coeff, mo_occ, with_t2=WITH_T2,
    ...
    h1mo = [
        reduce(numpy.dot, (mo.conj().T, h1ao[k], mo))
        for k, mo in enumerate(C)
    ]

    # veff_core in active MO (per k)
    veff_core = make_veff_core(mp)   # list over k # veff_core là thế hiệu dụng trong từ core (ĐÓNG BĂNG, năng lượng thấp)
    ...
    # Active BCH
    c0, c1 = active_BCH(
        mp, mp.mo_energy, mp.mo_coeff, mp.fock_hf
    )
    for k in range(nkpts):
        fock[k] += (c1[k] + c1[k].T.conj())

    # Internal BCH 
    _, c1_inter = active_BCH(
        mp, mp.mo_energy, mp.mo_coeff, mp.fock_hf
    )
...
def active_BCH(mp, mo_energy, mo_coeff, fock_hf):
    ...
    tmp1 = numpy.zeros((nkpts,nkpts,nkpts,nkpts,
                         nocc,nvir,nocc,nvir), dtype=complex)
    tmp1_bar = numpy.zeros_like(tmp1)

    # --- Needed MO integrals ---
    h2mo_ovgg = numpy.zeros((nkpts,nkpts,nkpts,nkpts,
                              nocc,nvir,nmo,nmo), dtype=complex)
    h2mo_ovov = numpy.zeros((nkpts,nkpts,nkpts,nkpts,
                              nocc,nvir,nocc,nvir), dtype=complex)
    h2mo_ovog = numpy.zeros((nkpts,nkpts,nkpts,nkpts,
                              nocc,nvir,nocc,nmo), dtype=complex)
    h2mo_ovgv = numpy.zeros((nkpts,nkpts,nkpts,nkpts,
                              nocc,nvir,nmo,nvir), dtype=complex)
    ...
    for ki in range(nkpts):
        for kj in range(nkpts):
            for ka in range(nkpts):
                ...
                w_iajb = (
                    h2mo_ovov[ki,ka,kj,kb]
                    - 0.5 * h2mo_ovov[ki,kb,kj,ka].transpose(0,3,2,1)
                )
                ...
                tmp1_bar[ki,ka,kj,kb][occ_act,vir_act,occ_act,vir_act] = (
                    w_iajb[occ_act,vir_act,occ_act,vir_act]
                    / e_iajb[occ_act,vir_act,occ_act,vir_act]
                ).conj()
    ...
    c1[:,:,:,:nocc] += 2*numpy.einsum(
        'qweriajb, qwtriapb -> tepj', tmp1_bar, h2mo_ovgv)

    c1[:,:,nocc:,:] -= 2*numpy.einsum(
        'qwetiajb, qweriajp -> trbp', tmp1_bar, h2mo_ovog)

    c1[:,:,nocc:,:nocc] += 2*numpy.einsum(
        'qai, qqeriajb -> rebj',
        fock_hf[:,nocc:,:nocc].conj(), tmp1_bar)

    c1 = numpy.einsum('wwpq -> wpq', c1)

    c0_1st = -4*numpy.einsum('qweriajb,qweriajb',
                             tmp1_bar, h2mo_ovov).real / nkpts
    ...
    if mp.second_order:
        ...
        y1 = 2*numpy.einsum(
            'qai, qqeriajb -> erjb',
            fock_hf[:,nocc:,:nocc].conj(), tmp1_bar)
        c2[:,:,nocc:,:nocc] = numpy.einsum(
            'erjb, erqtjbkc -> tqck', y1, tmp1_bar.conj())

        # [2][3][8]
        y1 = numpy.einsum(
            'qca, wqrtickb -> wqrtiakb',
            fock_hf[:,nocc:,nocc:], tmp1_bar.conj())
        c2[:,:,:nocc,:nocc] += numpy.einsum(
            'wqrtialb, wqetiajb -> relj', y1, tmp1)
        c2[:,:,:nocc,:nocc] += numpy.einsum(
            'wqrtkajb, eqrtiajb -> weki', y1, tmp1)
        c2[:,:,nocc:,nocc:] -= numpy.einsum(
            'wqrtiajd, wqreiajb -> etbd', y1, tmp1)
        c0_2nd -= 4*numpy.einsum(
            'qweriajb,qweriajb ->', y1, tmp1).real

        # [4][6][7]
        y1 = numpy.einsum(
            'qki, qwrtkalb -> qwrtialb',
            fock_hf[:,:nocc,:nocc], tmp1_bar.conj())
        c2[:,:,:nocc,:nocc] -= numpy.einsum(
            'qwrtialb, qwetiajb -> relj', y1, tmp1)
        c2[:,:,nocc:,nocc:] += numpy.einsum(
            'qwrticjb, qertiajb -> ewac', y1, tmp1)
        c2[:,:,nocc:,nocc:] += numpy.einsum(
            'qwrtiajd, qwreiajb -> etbd', y1, tmp1)
        c0_2nd += 4*numpy.einsum(
            'qwrtiajb, qwrtiajb ->', y1, tmp1).real
        c0_2nd /= nkpts

        # [5]
        y1 = numpy.einsum(
            'qwrtiajb, ewrtkajb -> eqki', tmp1, tmp1_bar.conj())
        c2[:,:,:,:nocc] -= numpy.einsum(
            'qpi, eqki -> qepk', fock_hf[:,:,:nocc], y1)

        # [9]
        y1 = numpy.einsum(
            'qwrtiajb, qerticjb -> weac', tmp1, tmp1_bar.conj())
        c2[:,:,:,nocc:] -= numpy.einsum(
            'wpa, weac -> wepc', fock_hf[:,:,nocc:], y1)
...
def inter_BCH(mp, mo_energy, mo_coeff, fock_hf,
        ...
        for kj in range(nkpts):
            for ka in range(nkpts):
                ...
                tmp1_bar[ki,ka,kj,kb][occ_int,vir_int,occ_int,vir_int] = (
                    w_iajb[occ_int,vir_int,occ_int,vir_int]
                    / e_iajb[occ_int,vir_int,occ_int,vir_int]
                ).conj()
    ...
    c1[:,:,nocc:,:nocc] += 2*numpy.einsum(
        'qai,qqeriajb->rebj', fock_hf[:,nocc:,:nocc].conj(), tmp1_bar)
    ...
    if mp.second_order:
        ...
        c0_2nd -= 4*numpy.einsum(
            'qweriajb,qweriajb->', y1, tmp1).real / nkpts
