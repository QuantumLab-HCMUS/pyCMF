...
def concentric_localization(C_vir_eff, S, F, active_aos, n_shells=1, tol=1e-5, verbose=True):
    ...
    if verbose:
        print(f'   [CL] ── Tóm tắt ──────────────────────────────────────')
        print(f'   [CL]   Shells được dùng  : {n_shells_used}  (max có thể = {max_possible_shells})')
        print(f'   [CL]   Virtual giữ lại   : {n_kept} / {n_vir}  ({100 * n_kept / n_vir:.1f}%)')
        if C_ker.shape[1] > 0 and n_shells != -1:
            print(f'   [CL]   Virtual bị loại   : {n_vir - n_kept}  (kernel còn lại = {C_ker.shape[1]})')
        print(f'   [CL] ──────────────────────────────────────────────────')
...
def cl_shell_analysis(C_vir_eff, S, F, active_aos, tol=1e-5):
    ...
    print('=' * 55)
    concentric_localization(C_vir_eff, S, F, active_aos, n_shells=-1, tol=tol, verbose=True)
    print('=' * 55 + '\n')
