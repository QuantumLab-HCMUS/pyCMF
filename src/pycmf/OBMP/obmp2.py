...
def make_veff(mp):
    ...
    veff += 2.0 * numpy.einsum('ijkk -> ij', h2mo_ggoo) - numpy.einsum('ijjk -> ik', h2mo_goog)

    c0_hf = 0.0
    for i in range(nocc):
        for j in range(nocc):
            c0_hf -= 2.0 * h2mo_ggoo[i, i, j, j] - h2mo_ggoo[i, j, j, i]
...
def first_BCH(mp, fock_hf, tmp1, tmp1_bar, c0):
    ...
    c0 -= 4.0 * numpy.sum(h2mo_ovov * tmp1_bar)
    ####################### c1[j,B] #########################
    ...
    for j in range(nocc):
        c1[:, j] = 4 * numpy.einsum(
            'ijkl -> k', h2mo_ovgv * numpy.einsum('ijkl -> jkil', numpy.tile(tmp1_bar[:, :, j, :], (nmo, 1, 1, 1)))
        )

    ####################### c1[p,B] #########################
    for b in range(nvir):
        c1[:, b + nocc] = -4 * numpy.einsum(
            'ijkl -> l', h2mo_ovog * numpy.einsum('ijkl -> jkli', numpy.tile(tmp1_bar[:, :, :, b], (nmo, 1, 1, 1)))
        )
