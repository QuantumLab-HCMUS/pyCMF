...
def make_amp(mp):
    ...
    tmp1_aa = 1.0 * css * numpy.dot(qov_a.T, qov_a).reshape(nocca, nvira, nocca, nvira) / x_aa
    ...
    tmp1_ab = 1.0 * cos * numpy.dot(qov_a.T, qov_b).reshape(nocca, nvira, noccb, nvirb) / x_ab
    ...
    tmp1_ba = 1.0 * cos * numpy.dot(qov_b.T, qov_a).reshape(noccb, nvirb, nocca, nvira) / x_ba
    ...
    tmp1_bb = 1.0 * css * numpy.dot(qov_b.T, qov_b).reshape(noccb, nvirb, noccb, nvirb) / x_bb
...
def first_BCH(mp, fock_hfa, fock_hfb, tmp1_bar, c0):
    ...
    for istep, qgv_a in enumerate(mp.loop_ao2mo_cgcv(mo_coeff[0], nocca)):
        for i in range(nocca):
            c1_a[:, 0:nocca] += 2.0 * lib.einsum(
                'apb, ajb -> pj',
                numpy.dot(
                    qgv_a.reshape(naux, nmoa, nvira)[:, :nocca, :nvira]
                    .reshape(naux, nocca * nvira)[:, i * nvira : (i + 1) * nvira]
                    .T,
                    qgv_a,
                ).reshape(nvira, nmoa, nvira),
                tmp1_bar_aa[i, :, :, :],
            )
        for i in range(noccb):
            c1_a[:, 0:nocca] += 2.0 * lib.einsum(
                'apb, ajb -> pj',
                numpy.dot(qov_b[:, i * nvirb : (i + 1) * nvirb].T, qgv_a).reshape(nvirb, nmoa, nvira),
                tmp1_bar_ba[i, :, :, :],
            )
    ...
    for istep, qov_a in enumerate(mp.loop_ao2mo(mo_coeff[0], nocca)):
        for i in range(nocca):
            c0 -= 1.0 * lib.einsum(
                'ajb, ajb -> ',
                numpy.dot(qov_a[:, i * nvira : (i + 1) * nvira].T, qov_a).reshape(nvira, nocca, nvira),
                tmp1_bar_aa[i, :, :, :],
            )
    ...
    for i in range(nocca):
        c0 -= 1.0 * lib.einsum(
            'ajb, ajb -> ',
            numpy.dot(qov_a[:, i * nvira : (i + 1) * nvira].T, qov_b).reshape(nvira, noccb, nvirb),
            tmp1_bar_ab[i, :, :, :],
        )
    ...
    for istep, qgv_b in enumerate(mp.loop_ao2mo_cgcv(mo_coeff[1], noccb)):
        for i in range(noccb):
            c1_b[:, 0:noccb] += 2.0 * lib.einsum(
                'apb, ajb -> pj',
                numpy.dot(
                    qgv_b.reshape(naux, nmob, nvirb)[:, :noccb, :nvirb]
                    .reshape(naux, noccb * nvirb)[:, i * nvirb : (i + 1) * nvirb]
                    .T,
                    qgv_b,
                ).reshape(nvirb, nmob, nvirb),
                tmp1_bar_bb[i, :, :, :],
            )
        for i in range(nocca):
            c1_b[:, 0:noccb] += 2.0 * lib.einsum(
                'apb, ajb -> pj',
                numpy.dot(qov_a[:, i * nvira : (i + 1) * nvira].T, qgv_b).reshape(nvira, nmob, nvirb),
                tmp1_bar_ab[i, :, :, :],
            )
    ...
    for i in range(noccb):
        c0 -= 1.0 * lib.einsum(
            'ajb, ajb -> ',
            numpy.dot(qov_b[:, i * nvirb : (i + 1) * nvirb].T, qov_a).reshape(nvirb, nocca, nvira),
            tmp1_bar_ba[i, :, :, :],
        )
        c0 -= 1.0 * lib.einsum(
            'ajb, ajb -> ',
            numpy.dot(qov_b[:, i * nvirb : (i + 1) * nvirb].T, qov_b).reshape(nvirb, noccb, nvirb),
            tmp1_bar_bb[i, :, :, :],
        )
    ...
    for istep, qog_a in enumerate(mp.loop_ao2mo_goog_cocg(mo_coeff[0], nocca)):
        for i in range(nocca):
            c1_a[:, nocca:nmoa] -= 2.0 * lib.einsum(
                'ajp, ajb -> pb',
                numpy.dot(qov_a[:, i * nvira : (i + 1) * nvira].T, qog_a).reshape(nvira, nocca, nmoa),
                tmp1_bar_aa[i, :, :, :],
            )
        for i in range(noccb):
            c1_a[:, nocca:nmoa] -= 2.0 * lib.einsum(
                'ajp, ajb -> pb',
                numpy.dot(qov_b[:, i * nvirb : (i + 1) * nvirb].T, qog_a).reshape(nvirb, nocca, nmoa),
                tmp1_bar_ba[i, :, :, :],
            )
    ...
    for istep, qog_b in enumerate(mp.loop_ao2mo_goog_cocg(mo_coeff[1], noccb)):
        for i in range(noccb):
            c1_b[:, noccb:nmob] -= 2.0 * lib.einsum(
                'ajp, ajb -> pb',
                numpy.dot(qov_b[:, i * nvirb : (i + 1) * nvirb].T, qog_b).reshape(nvirb, noccb, nmob),
                tmp1_bar_bb[i, :, :, :],
            )
        for i in range(nocca):
            c1_b[:, noccb:nmob] -= 2.0 * lib.einsum(
                'ajp, ajb -> pb',
                numpy.dot(qov_a[:, i * nvira : (i + 1) * nvira].T, qog_b).reshape(nvira, noccb, nmob),
                tmp1_bar_ab[i, :, :, :],
            )
...
if __name__ == '__main__':
    ...
    mol = gto.Mole()
    mol.atom = [[17, (0.0, -0.757, 0.587)], [17, (0.0, 0.757, 0.587)]]
