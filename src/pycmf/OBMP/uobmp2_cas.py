...
def kernel(mp, mo_energy=None, mo_coeff=None, eris=None, with_t2=WITH_T2, verbose=logger.NOTE):
    ...
    nocc_inact = numpy.array([nocc[0] - nocc_act[0], nocc[1] - nocc_act[1]])
    ...
    h1mo_act = numpy.array([numpy.zeros([nact[0], nact[0]]), numpy.zeros([nact[1], nact[1]])])
    for sp in numpy.arange(2):
        h1mo = numpy.matmul(mp.mo_coeff[sp].T, numpy.matmul(h1ao, mp.mo_coeff[sp]))
        h1mo_act[sp] = h1mo[nocc_inact[sp] : nocc_inact[sp] + nact[sp], nocc_inact[sp] : nocc_inact[sp] + nact[sp]]
    ...
    cg = [
        mo_coeff[0][:, nocc_inact[0] : nocc_inact[0] + nact[0]],
    ...
    for sa in numpy.arange(2):
        for sb in numpy.arange(2):
            tmp1[ele] = mp.tmp1[ele][
                nocc_inact[sa] : nocc_inact[sa] + nocc_act[sa],
                : nvir_act[sa],
                nocc_inact[sb] : nocc_inact[sb] + nocc_act[sb],
                : nvir_act[sb],
            ]
            tmp1_bar[ele] = mp.tmp1_bar[ele][
                nocc_inact[sa] : nocc_inact[sa] + nocc_act[sa],
                : nvir_act[sa],
                nocc_inact[sb] : nocc_inact[sb] + nocc_act[sb],
                : nvir_act[sb],
...
def active_tmp1(mp, occ_ele, oact_ele, t2_ele, caslist_ele):
    ...
    if len(ext_idx[0]) + mp.nact[0] != nmo[0] or len(ext_idx[1]) + mp.nact[1] != nmo[1]:
        raise ValueError(
    ...
    """
    t2_rcore = t2_ele\
        [ext_idx[0][:nocc_inact[0]],:,ext_idx[1][:nocc_inact[1]],:]\
            .reshape(nocc_inact[0],nvir[0],nocc_inact[1],nvir[1])

    t2_ract = t2_ele[occ_idx[0],:, occ_idx[1],:]\
                .reshape(occ_len[0], nvir[0], occ_len[1], nvir[1])

    t2_row = numpy.stack([t2_rcore, t2_ract])\
                #.reshape(occ_ele[0], nvir[0], occ_ele[1], nvir[1])
    print(t2_rcore.shape)
    print(t2_row.shape)

    t2_act = t2_row[:, vir_idx[0],:, vir_idx[1]]
    t2_ext = t2_row[:, ext_idx[0][nocc_inact[0]:]
                , :, ext_idx[1][nocc_inact[1]:]]
    t2_ele = numpy.block([t2_act, t2_ext])
    """
    return t2_l
...
def sort_tmp1(mp, tmp1, caslist):
    ...
    for sa in numpy.arange(2):
        for sb in numpy.arange(2):
            occ_ele = [nocc[sa], nocc[sb]]
            oact_ele = [nocc_act[sa], nocc_act[sb]]
            caslist_ele = [caslist[sa], caslist[sb]]
            tmp1_sorted[ele] = active_tmp1(mp, occ_ele, oact_ele, tmp1[ele], caslist_ele)
            ele += 1
...
def make_veff_core(mp):
    ...
    cc = [mo_coeff[0][:, : nocc_inact[0]], mo_coeff[1][:, : nocc_inact[1]]]
    ...
    veff_core = numpy.array([numpy.zeros((nact[0], nact[0])), numpy.zeros((nact[1], nact[1]))])
    veff_core[0] += (
        numpy.einsum('ijkk -> ij', h2mo_aa_ggcc)
        - numpy.einsum('ijjk -> ik', h2mo_aa_gccg)
        + numpy.einsum('ijkk -> ij', h2mo_ab_ggcc)
    )
    veff_core[1] += (
        numpy.einsum('ijkk -> ij', h2mo_bb_ggcc)
...
def make_veff(mp):
    ...
    co = [
        mo_coeff[0][:, nocc_inact[0] : nocc_inact[0] + nocc_act[0]],
    ...
    veff = numpy.array([numpy.zeros((nact[0], nact[0])), numpy.zeros((nact[1], nact[1]))])
    veff[0] += (
...
def make_amp(mp):
    ...
    h2mo_act = [
        numpy.zeros([nocc_act[0], nvir_act[0], nocc_act[0], nvir_act[0]]),
    ...
    x = numpy.tile(
        mo_energy_act[0][: nocc_act[0], None] - mo_energy_act[0][None, nocc_act[0] :], (nocc_act[0], nvir_act[0], 1, 1)
    )
    ...
    x = numpy.einsum(
        'ijkl -> klij',
        numpy.tile(
            mo_energy_act[0][: nocc_act[0], None] - mo_energy_act[0][None, nocc_act[0] :],
            (nocc_act[1], nvir_act[1], 1, 1),
        ),
    )
    x += (
        numpy.tile(
            mo_energy_act[1][: nocc_act[1], None] - mo_energy_act[1][None, nocc_act[1] :],
            (nocc_act[0], nvir_act[0], 1, 1),
        )
        - mp.shift
    )
    ...
    x = numpy.tile(
        mo_energy_act[1][: nocc_act[1], None] - mo_energy_act[1][None, nocc_act[1] :], (nocc_act[1], nvir_act[1], 1, 1)
    )
    x += numpy.einsum('ijkl -> klij', x) - mp.shift
...
def first_BCH(mp, fock_hf, tmp1_bar, c0):
    ...
    c0 -= 1.0 * numpy.sum(h2mo_aa_ovov * tmp1_bar[0])
    c0 -= 1.0 * numpy.sum(h2mo_ab_ovov * tmp1_bar[1])
    c0 -= 1.0 * numpy.sum(h2mo_ba_ovov * tmp1_bar[2])
    c0 -= 1.0 * numpy.sum(h2mo_bb_ovov * tmp1_bar[3])

    ####################### c1_a[j,B] #########################
    c1_a[: nocc_act[0], nocc_act[0] :] += 2 * numpy.einsum(
        'ijkl -> ij',
        numpy.einsum('ijkl -> klij', tmp1_bar[0])
        * numpy.tile(fock_hf[0][: nocc_act[0], nocc_act[0] :], (nocc_act[0], nvir_act[0], 1, 1)),
    )
    c1_a[: nocc_act[0], nocc_act[0] :] += 2 * numpy.einsum(
        'ijkl -> ij',
        numpy.einsum('ijkl -> klij', tmp1_bar[2])
    ...
    for j in range(nocc_act[0]):
        c1_a[:, j] += 2 * numpy.einsum(
            'ijkl -> k',
            h2mo_aa_ovgv * numpy.einsum('ijkl -> jkil', numpy.tile(tmp1_bar[0][:, :, j, :], (nact[0], 1, 1, 1))),
        )
        c1_a[:, j] += 2 * numpy.einsum(
            'ijkl -> k',
            h2mo_ba_ovgv * numpy.einsum('ijkl -> jkil', numpy.tile(tmp1_bar[2][:, :, j, :], (nact[0], 1, 1, 1))),
        )

    ####################### c1_a[p,B] #########################
    for b in range(nvir_act[0]):
        c1_a[:, b + nocc_act[0]] -= 2 * numpy.einsum(
            'ijkl -> l',
            h2mo_aa_ovog * numpy.einsum('ijkl -> jkli', numpy.tile(tmp1_bar[0][:, :, :, b], (nact[0], 1, 1, 1))),
        )
        c1_a[:, b + nocc_act[0]] -= 2 * numpy.einsum(
            'ijkl -> l',
            h2mo_ba_ovog * numpy.einsum('ijkl -> jkli', numpy.tile(tmp1_bar[2][:, :, :, b], (nact[0], 1, 1, 1))),
        )

    ####################### c1_b[j,B] #########################
    c1_b[: nocc_act[1], nocc_act[1] :] += 2 * numpy.einsum(
        'ijkl -> ij',
        numpy.einsum('ijkl -> klij', tmp1_bar[3])
        * numpy.tile(fock_hf[1][: nocc_act[1], nocc_act[1] :], (nocc_act[1], nvir_act[1], 1, 1)),
    )
    c1_b[: nocc_act[1], nocc_act[1] :] += 2 * numpy.einsum(
        'ijkl -> ij',
        numpy.einsum('ijkl -> klij', tmp1_bar[1])
    ...
    for j in range(nocc_act[1]):
        c1_b[:, j] += 2 * numpy.einsum(
            'ijkl -> k',
            h2mo_bb_ovgv * numpy.einsum('ijkl -> jkil', numpy.tile(tmp1_bar[3][:, :, j, :], (nact[1], 1, 1, 1))),
        )
        c1_b[:, j] += 2 * numpy.einsum(
            'ijkl -> k',
            h2mo_ab_ovgv * numpy.einsum('ijkl -> jkil', numpy.tile(tmp1_bar[1][:, :, j, :], (nact[1], 1, 1, 1))),
        )
    ####################### c1_a[p,B] #########################
    for b in range(nvir_act[1]):
        c1_b[:, b + nocc_act[1]] -= 2 * numpy.einsum(
            'ijkl -> l',
            h2mo_bb_ovog * numpy.einsum('ijkl -> jkli', numpy.tile(tmp1_bar[3][:, :, :, b], (nact[1], 1, 1, 1))),
        )
        c1_b[:, b + nocc_act[1]] -= 2 * numpy.einsum(
            'ijkl -> l',
            h2mo_ab_ovog * numpy.einsum('ijkl -> jkli', numpy.tile(tmp1_bar[1][:, :, :, b], (nact[1], 1, 1, 1))),
        )
...
def second_BCH(mp, fock, fock_hf, tmp1, tmp1_bar, c0):
    ...
    y1_a = numpy.einsum(
        'ijkl -> kl',
        numpy.einsum(
            'ijkl -> klij', numpy.tile(fock_hf[0][: nocc_act[0], nocc_act[0] :], (nocc_act[0], nvir_act[0], 1, 1))
        )
        * tmp1_bar[0],
    )
    ...
    y1_a += numpy.einsum(
        'ijkl -> kl',
        numpy.einsum(
            'ijkl -> klij', numpy.tile(fock_hf[1][: nocc_act[1], nocc_act[1] :], (nocc_act[0], nvir_act[0], 1, 1))
        )
        * tmp1_bar[2],
    )

    c1_a[: nocc_act[0], nocc_act[0] :] += 1.0 * numpy.einsum(
        'ijkl -> ij', numpy.tile(y1_a, (nocc_act[0], nvir_act[0], 1, 1)) * tmp1_bar[0]
    )

    c1_b[: nocc_act[1], nocc_act[1] :] += 1.0 * numpy.einsum(
        'ijkl -> ij', numpy.tile(y1_a, (nocc_act[1], nvir_act[1], 1, 1)) * tmp1_bar[2]
    )

    y1_b = numpy.einsum(
        'ijkl -> kl',
        numpy.einsum(
            'ijkl -> klij', numpy.tile(fock_hf[1][: nocc_act[1], nocc_act[1] :], (nocc_act[1], nvir_act[1], 1, 1))
        )
        * tmp1_bar[3],
    )
    ...
    y1_b += numpy.einsum(
        'ijkl -> kl',
        numpy.einsum(
            'ijkl -> klij', numpy.tile(fock_hf[0][: nocc_act[0], nocc_act[0] :], (nocc_act[1], nvir_act[1], 1, 1))
        )
        * tmp1_bar[1],
    )

    c1_a[: nocc_act[0], nocc_act[0] :] += 1.0 * numpy.einsum(
        'ijkl -> ij', numpy.tile(y1_b, (nocc_act[0], nvir_act[0], 1, 1)) * tmp1_bar[1]
    )
    c1_b[: nocc_act[1], nocc_act[1] :] += 1.0 * numpy.einsum(
        'ijkl -> ij', numpy.tile(y1_b, (nocc_act[1], nvir_act[1], 1, 1)) * tmp1_bar[3]
    )
    ...
    for c in range(nvir_act[0]):
        y1_aa += numpy.einsum(
            'ijkl -> klij',
            numpy.tile(fock_hf[0][nocc_act[0] :, c - nvir_act[0]].T, (nocc_act[0], nvir_act[0], nocc_act[0], 1)),
        ) * numpy.einsum('ijkl ->jikl', numpy.tile(tmp1_bar[0][:, c, :, :], (nvir_act[0], 1, 1, 1)))
        y1_ab += numpy.einsum(
            'ijkl -> ilkj',
            numpy.tile(fock_hf[0][nocc_act[0] :, c - nvir_act[0]].T, (nocc_act[0], nvir_act[1], nocc_act[1], 1)),
        ) * numpy.einsum('ijkl ->jikl', numpy.tile(tmp1_bar[1][:, c, :, :], (nvir_act[0], 1, 1, 1)))

    for c in range(nvir_act[1]):
        y1_ba += numpy.einsum(
            'ijkl -> ilkj',
            numpy.tile(fock_hf[1][nocc_act[1] :, c - nvir_act[1]].T, (nocc_act[1], nvir_act[0], nocc_act[0], 1)),
        ) * numpy.einsum('ijkl ->jikl', numpy.tile(tmp1_bar[2][:, c, :, :], (nvir_act[1], 1, 1, 1)))
        y1_bb += numpy.einsum(
            'ijkl -> klij',
            numpy.tile(fock_hf[1][nocc_act[1] :, c - nvir_act[1]].T, (nocc_act[1], nvir_act[1], nocc_act[1], 1)),
        ) * numpy.einsum('ijkl ->jikl', numpy.tile(tmp1_bar[3][:, c, :, :], (nvir_act[1], 1, 1, 1)))

    for k in range(nocc_act[0]):
        c1_a[: nocc_act[0], k] += 1.0 * numpy.einsum(
            'ijkl -> k', tmp1[0] * numpy.einsum('ijkl -> jkil', numpy.tile(y1_aa[:, :, k, :], (nocc_act[0], 1, 1, 1)))
        )
        c1_a[: nocc_act[0], k] += 1.0 * numpy.einsum(
            'ijkl -> k', tmp1[2] * numpy.einsum('ijkl -> jkil', numpy.tile(y1_ba[:, :, k, :], (nocc_act[0], 1, 1, 1)))
        )

    for k in range(nocc_act[1]):
        c1_b[: nocc_act[1], k] += 1.0 * numpy.einsum(
            'ijkl -> k', tmp1[3] * numpy.einsum('ijkl -> jkil', numpy.tile(y1_bb[:, :, k, :], (nocc_act[1], 1, 1, 1)))
        )
        c1_b[: nocc_act[1], k] += 1.0 * numpy.einsum(
            'ijkl -> k', tmp1[1] * numpy.einsum('ijkl -> jkil', numpy.tile(y1_ab[:, :, k, :], (nocc_act[1], 1, 1, 1)))
        )

    c0 -= 1.0 * numpy.sum(tmp1[0] * y1_aa) + 1.0 * numpy.sum(tmp1[3] * y1_bb)
    c0 -= 1.0 * numpy.sum(tmp1[1] * y1_ab) + 1.0 * numpy.sum(tmp1[2] * y1_ba)
    ...
    for k in range(nocc_act[0]):
        c1_a[: nocc_act[0], k] += 1.0 * numpy.einsum(
            'ijkl -> i', tmp1[0] * numpy.tile(y1_aa[k, :, :, :], (nocc_act[0], 1, 1, 1))
        )
        c1_a[: nocc_act[0], k] += 1.0 * numpy.einsum(
            'ijkl -> i', tmp1[1] * numpy.tile(y1_ab[k, :, :, :], (nocc_act[0], 1, 1, 1))
        )
    ...
    for k in range(nocc_act[1]):
        c1_b[: nocc_act[1], k] += 1.0 * numpy.einsum(
            'ijkl -> i', tmp1[3] * numpy.tile(y1_bb[k, :, :, :], (nocc_act[1], 1, 1, 1))
        )
    for k in range(nocc_act[1]):
        if mp.break_sym:
            c1_b[: nocc_act[1], k] += 1.0 * numpy.einsum(
                'ijkl -> i',
                tmp1[2] * numpy.einsum('ijkl -> jkil', numpy.tile(y1_ba[:, :, k, :], (nocc_act[0], 1, 1, 1))),
            )
        else:
            c1_b[: nocc_act[1], k] += 1.0 * numpy.einsum(
                'ijkl -> i', tmp1[2] * numpy.tile(y1_ba[k, :, :, :], (nocc_act[1], 1, 1, 1))
            )
    ...
    for k in range(nocc_act[0]):
        y1_aa += numpy.einsum(
            'ijkl -> ljik', numpy.tile(fock_hf[0][: nocc_act[0], k], (nocc_act[0], nvir_act[0], nvir_act[0], 1))
        ) * numpy.tile(tmp1_bar[0][k, :, :, :], (nocc_act[0], 1, 1, 1))
        y1_ab += numpy.einsum(
            'ijkl -> lkij', numpy.tile(fock_hf[0][: nocc_act[0], k], (nocc_act[1], nvir_act[1], nvir_act[0], 1))
        ) * numpy.tile(tmp1_bar[1][k, :, :, :], (nocc_act[0], 1, 1, 1))

    for k in range(nocc_act[1]):
        y1_ba += numpy.einsum(
            'ijkl -> lkij', numpy.tile(fock_hf[1][: nocc_act[1], k], (nocc_act[0], nvir_act[0], nvir_act[1], 1))
        ) * numpy.tile(tmp1_bar[2][k, :, :, :], (nocc_act[1], 1, 1, 1))
        y1_bb += numpy.einsum(
            'ijkl -> ljik', numpy.tile(fock_hf[1][: nocc_act[1], k], (nocc_act[1], nvir_act[1], nvir_act[1], 1))
    ...
    for k in range(nocc_act[0]):
        y1_a[:, k] += numpy.einsum('ijkl -> i', tmp1[0] * numpy.tile(tmp1_bar[0][k, :, :, :], (nocc_act[0], 1, 1, 1)))
        y1_a[:, k] += numpy.einsum('ijkl -> i', tmp1[1] * numpy.tile(tmp1_bar[1][k, :, :, :], (nocc_act[0], 1, 1, 1)))

    for k in range(nocc_act[0]):
        c1_a[:, k] -= 1.0 * numpy.einsum(
            'ij -> i', fock_hf[0][: nocc_act[0], :].T * numpy.tile(y1_a[:, k], (nact[0], 1))
        )

    for k in range(nocc_act[1]):
        y1_b[:, k] += numpy.einsum('ijkl -> i', tmp1[3] * numpy.tile(tmp1_bar[3][k, :, :, :], (nocc_act[1], 1, 1, 1)))
        y1_b[:, k] += numpy.einsum('ijkl -> i', tmp1[2] * numpy.tile(tmp1_bar[2][k, :, :, :], (nocc_act[1], 1, 1, 1)))

    for k in range(nocc_act[1]):
        c1_b[:, k] -= 1.0 * numpy.einsum(
            'ij -> i', fock_hf[1][: nocc_act[1], :].T * numpy.tile(y1_b[:, k], (nact[1], 1))
        )
    ...
    for c in range(nvir_act[0]):
        c1_a[nocc_act[0] :, c + nocc_act[0]] += 1.0 * numpy.einsum(
            'ijkl -> l', tmp1[0] * numpy.einsum('ijkl -> jkli', numpy.tile(y1_aa[:, :, :, c], (nvir_act[0], 1, 1, 1)))
        )
        c1_a[nocc_act[0] :, c + nocc_act[0]] += 1.0 * numpy.einsum(
            'ijkl -> l', tmp1[2] * numpy.einsum('ijkl -> jkli', numpy.tile(y1_ba[:, :, :, c], (nvir_act[0], 1, 1, 1)))
        )

    for c in range(nvir_act[1]):
        c1_b[nocc_act[1] :, c + nocc_act[1]] += 1.0 * numpy.einsum(
            'ijkl -> l', tmp1[3] * numpy.einsum('ijkl -> jkli', numpy.tile(y1_bb[:, :, :, c], (nvir_act[1], 1, 1, 1)))
        )
        c1_b[nocc_act[1] :, c + nocc_act[1]] += 1.0 * numpy.einsum(
            'ijkl -> l', tmp1[1] * numpy.einsum('ijkl -> jkli', numpy.tile(y1_ab[:, :, :, c], (nvir_act[1], 1, 1, 1)))
        )
    ...
    for c in range(nvir_act[0]):
        c1_a[nocc_act[0] :, c + nocc_act[0]] += 1.0 * numpy.einsum(
            'ijkl -> j', tmp1[0] * numpy.einsum('ijkl -> jikl', numpy.tile(y1_aa[:, c, :, :], (nvir_act[0], 1, 1, 1)))
        )
        c1_a[nocc_act[0] :, c + nocc_act[0]] += 1.0 * numpy.einsum(
            'ijkl -> j', tmp1[1] * numpy.einsum('ijkl -> jikl', numpy.tile(y1_ab[:, c, :, :], (nvir_act[0], 1, 1, 1)))
        )
    ...
    for c in range(nvir_act[1]):
        c1_b[nocc_act[1] :, c + nocc_act[1]] += 1.0 * numpy.einsum(
            'ijkl -> j', tmp1[3] * numpy.einsum('ijkl -> jikl', numpy.tile(y1_bb[:, c, :, :], (nvir_act[1], 1, 1, 1)))
        )
        c1_b[nocc_act[1] :, c + nocc_act[1]] += 1.0 * numpy.einsum(
            'ijkl -> j', tmp1[2] * numpy.einsum('ijkl -> jikl', numpy.tile(y1_ba[:, c, :, :], (nvir_act[1], 1, 1, 1)))
        )
    ...
    for c in range(nvir_act[0]):
        y1_a[:, c] += numpy.einsum(
            'ijkl -> j',
            tmp1[0] * numpy.einsum('ijkl -> jikl', numpy.tile(tmp1_bar[0][:, c, :, :], (nvir_act[0], 1, 1, 1))),
        )
        y1_a[:, c] += numpy.einsum(
            'ijkl -> j',
            tmp1[1] * numpy.einsum('ijkl -> jikl', numpy.tile(tmp1_bar[1][:, c, :, :], (nvir_act[0], 1, 1, 1))),
        )

    for c in range(nvir_act[0]):
        c1_a[:, c + nocc_act[0]] -= 1.0 * numpy.einsum(
            'ij -> i', fock_hf[0][nocc_act[0] :, :].T * numpy.tile(y1_a[:, c], (nact[0], 1))
        )

    for c in range(nvir_act[1]):
        y1_b[:, c] += numpy.einsum(
            'ijkl -> j',
            tmp1[3] * numpy.einsum('ijkl -> jikl', numpy.tile(tmp1_bar[3][:, c, :, :], (nvir_act[1], 1, 1, 1))),
        )
        y1_b[:, c] += numpy.einsum(
            'ijkl -> j',
            tmp1[2] * numpy.einsum('ijkl -> jikl', numpy.tile(tmp1_bar[2][:, c, :, :], (nvir_act[1], 1, 1, 1))),
        )

    for c in range(nvir_act[1]):
        c1_b[:, c + nocc_act[1]] -= 1.0 * numpy.einsum(
            'ij -> i', fock_hf[1][nocc_act[1] :, :].T * numpy.tile(y1_b[:, c], (nact[1], 1))
        )
    return c0, c1_a, c1_b
...
def make_rdm1(mp, use_t2=False, use_ao=True, **kwargs):
    ...
    if not use_t2:
        ...
        if use_ao:
            rdm1_ao = (
                reduce(numpy.dot, (mo_cas[0], rdm1[0], mo_cas[0].T)),
...
class UOBMP2(obmp2_cas.OBMP2):
    ...
    def kernel(self, mo_energy=None, mo_coeff=None, eris=None, with_t2=WITH_T2, _kern=kernel):
        self.ene_act, self.h1mo_vqe, self.h2mo_act, self.tmp1_bar_act, self.tmp1 = _kern(
            self, mo_energy, mo_coeff, eris, with_t2
        )
