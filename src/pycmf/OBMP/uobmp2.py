...
def kernel(mp, mo_energy=None, mo_coeff=None, eris=None, with_t2=WITH_T2, verbose=logger.NOTE):
    ...
    for it in range(niter):
        ...
        h2mo_a = ao2mo.general(mp._scf._eri, (cg_a, cg_a, cg_a, cg_a), compact=False)
        h2mo_a = h2mo_a.reshape(nmoa, nmoa, nmoa, nmoa)

        h2mo_b = ao2mo.general(mp._scf._eri, (cg_b, cg_b, cg_b, cg_b), compact=False)
        h2mo_b = h2mo_b.reshape(nmob, nmob, nmob, nmob)

        h2mo_ab = ao2mo.general(mp._scf._eri, (cg_a, cg_a, cg_b, cg_b), compact=False)
        h2mo_ab = h2mo_ab.reshape(nmoa, nmoa, nmob, nmob)

        h2mo_ba = ao2mo.general(mp._scf._eri, (cg_b, cg_b, cg_a, cg_a), compact=False)
        h2mo_ba = h2mo_ba.reshape(nmob, nmob, nmoa, nmoa)
        ...
        ####################
        #### MP1 amplitude
        tmp1_aa, tmp1_bb, tmp1_ab, tmp1_ba, tmp1_bar_aa, tmp1_bar_bb, tmp1_bar_ab, tmp1_bar_ba = make_amp(mp)

        tmp1_bar = [0, 0, 0, 0]
        tmp1_bar[0] += tmp1_bar_aa
        tmp1_bar[1] += tmp1_bar_ab
        tmp1_bar[2] += tmp1_bar_ba
        tmp1_bar[3] += tmp1_bar_bb
        ...
        ### BCH 2nd order
        if mp.second_order:
            c0, c1_a_2nd, c1_b_2nd = second_BCH(
                mp,
                fock_hfa,
                fock_hfb,
                tmp1_aa,
            ...
            # symmetrize c1
            fock_a += 0.5 * (c1_a_2nd + c1_a_2nd.T)
            fock_b += 0.5 * (c1_b_2nd + c1_b_2nd.T)

        print('trace = ', numpy.trace(c1_a[:1, :1] + c1_a[6:, 6:]))
        c0_tot = 0
        c0_tot += c0

        # print("fock_a f:", fock_a)
        ...
        """
        # === Spin penalty correction ===
        lambda_spin = getattr(mp, "lambda_spin_pen", 0.01)  # penalty strength, adjust as needed

        #nocca, noccb = mp.get_nocc()
        #nmoa, nmob = fock_a.shape[0], fock_b.shape[0]

        # Build density matrices in AO basis from current mo_coeff (MOM-updated)
        C_a_occ = mo_coeff[0][:, :nocca]   # occupied alpha orbitals (reordered by MOM)
        C_b_occ = mo_coeff[1][:, :noccb]   # occupied beta orbitals
        D_a = C_a_occ @ C_a_occ.T
        D_b = C_b_occ @ C_b_occ.T
        ...
        # Apply penalty to Fock matrices
        fock_a += 2.0 * lambda_spin * DeltaD
        fock_b -= 2.0 * lambda_spin * DeltaD

        # Compute penalty energy (for monitoring only)
        E_pen = lambda_spin * numpy.einsum('ij,ji->', DeltaD, DeltaD)
        print(f"Iter {it}:  spin penalty E_pen = {E_pen:.6e}")
        # ===============================
        """
    ...
    if mp.second_order:
        ...
        c1[1] += c1_b_2nd
    fock = [fock_a, fock_b]
    h1mo = [h1mo_a_vqe, h1mo_b_vqe]
    h2mo = [h2mo_a, h2mo_ab, h2mo_ba, h2mo_b]
    tmp1 = [tmp1_aa, tmp1_ab, tmp1_ba, tmp1_bb]

    return ene_tot, h1mo, h2mo, tmp1_bar, tmp1, fock_hf, fock, fock_1st, c0_tot, c1, ss
...
def make_amp(mp):
    ...
    co_a = mo_coeff[0][:, :nocca]
    cv_a = mo_coeff[0][:, nocca:]
    co_b = mo_coeff[1][:, :noccb]
    cv_b = mo_coeff[1][:, noccb:]
    ...
    tmp1_aa = 1.0 * h2mo_aa / x

    x = numpy.tile(mo_energy[1][:noccb, None] - mo_energy[1][None, noccb:], (noccb, nvirb, 1, 1))
    x += numpy.einsum('ijkl -> klij', x) - mp.shift
    tmp1_bb = 1.0 * h2mo_bb / x

    x = numpy.einsum(
        'ijkl -> klij', numpy.tile(mo_energy[0][:nocca, None] - mo_energy[0][None, nocca:], (noccb, nvirb, 1, 1))
    )
    x += numpy.tile(mo_energy[1][:noccb, None] - mo_energy[1][None, noccb:], (nocca, nvira, 1, 1)) - mp.shift
    tmp1_ab = 1.0 * h2mo_ab / x

    x = numpy.einsum(
        'ijkl -> klij', numpy.tile(mo_energy[1][:noccb, None] - mo_energy[1][None, noccb:], (nocca, nvira, 1, 1))
    )
    x += numpy.tile(mo_energy[0][:nocca, None] - mo_energy[0][None, nocca:], (noccb, nvirb, 1, 1)) - mp.shift
    tmp1_ba = 1.0 * h2mo_ba / x
...
def first_BCH(mp, fock_hfa, fock_hfb, tmp1_bar_aa, tmp1_bar_bb, tmp1_bar_ab, tmp1_bar_ba, c0):
    ...
    c0 -= 1.0 * numpy.sum(h2mo_aa_ovov * tmp1_bar_aa)
    c0 -= 1.0 * numpy.sum(h2mo_ab_ovov * tmp1_bar_ab)
    c0 -= 1.0 * numpy.sum(h2mo_ba_ovov * tmp1_bar_ba)
    c0 -= 1.0 * numpy.sum(h2mo_bb_ovov * tmp1_bar_bb)

    ####################### c1_a[j,B] #########################
    c1_a[:nocca, nocca:] += 2 * numpy.einsum(
        'ijkl -> ij',
        numpy.einsum('ijkl -> klij', tmp1_bar_aa) * numpy.tile(fock_hfa[:nocca, nocca:], (nocca, nvira, 1, 1)),
    )
    c1_a[:nocca, nocca:] += 2 * numpy.einsum(
        'ijkl -> ij',
        numpy.einsum('ijkl -> klij', tmp1_bar_ba) * numpy.tile(fock_hfb[:noccb, noccb:], (nocca, nvira, 1, 1)),
    )

    ####################### c1_a[p,j] #########################
    for j in range(nocca):
        c1_a[:, j] += 2 * numpy.einsum(
            'ijkl -> k',
            h2mo_aa_ovgv * numpy.einsum('ijkl -> jkil', numpy.tile(tmp1_bar_aa[:, :, j, :], (nmoa, 1, 1, 1))),
        )
        c1_a[:, j] += 2 * numpy.einsum(
            'ijkl -> k',
            h2mo_ba_ovgv * numpy.einsum('ijkl -> jkil', numpy.tile(tmp1_bar_ba[:, :, j, :], (nmoa, 1, 1, 1))),
        )

    ####################### c1_a[p,B] #########################
    for b in range(nvira):
        c1_a[:, b + nocca] -= 2 * numpy.einsum(
            'ijkl -> l',
            h2mo_aa_ovog * numpy.einsum('ijkl -> jkli', numpy.tile(tmp1_bar_aa[:, :, :, b], (nmoa, 1, 1, 1))),
        )
        c1_a[:, b + nocca] -= 2 * numpy.einsum(
            'ijkl -> l',
            h2mo_ba_ovog * numpy.einsum('ijkl -> jkli', numpy.tile(tmp1_bar_ba[:, :, :, b], (nmoa, 1, 1, 1))),
        )

    ####################### c1_b[j,B] #########################
    c1_b[:noccb, noccb:] += 2 * numpy.einsum(
        'ijkl -> ij',
        numpy.einsum('ijkl -> klij', tmp1_bar_bb) * numpy.tile(fock_hfb[:noccb, noccb:], (noccb, nvirb, 1, 1)),
    )
    c1_b[:noccb, noccb:] += 2 * numpy.einsum(
        'ijkl -> ij',
        numpy.einsum('ijkl -> klij', tmp1_bar_ab) * numpy.tile(fock_hfa[:nocca, nocca:], (noccb, nvirb, 1, 1)),
    )

    ####################### c1_b[p,j] #########################
    for j in range(noccb):
        c1_b[:, j] += 2 * numpy.einsum(
            'ijkl -> k',
            h2mo_bb_ovgv * numpy.einsum('ijkl -> jkil', numpy.tile(tmp1_bar_bb[:, :, j, :], (nmob, 1, 1, 1))),
        )
        c1_b[:, j] += 2 * numpy.einsum(
            'ijkl -> k',
            h2mo_ab_ovgv * numpy.einsum('ijkl -> jkil', numpy.tile(tmp1_bar_ab[:, :, j, :], (nmob, 1, 1, 1))),
        )

    ####################### c1_a[p,B] #########################
    for b in range(nvirb):
        c1_b[:, b + noccb] -= 2 * numpy.einsum(
            'ijkl -> l',
            h2mo_bb_ovog * numpy.einsum('ijkl -> jkli', numpy.tile(tmp1_bar_bb[:, :, :, b], (nmob, 1, 1, 1))),
        )
        c1_b[:, b + noccb] -= 2 * numpy.einsum(
            'ijkl -> l',
            h2mo_ab_ovog * numpy.einsum('ijkl -> jkli', numpy.tile(tmp1_bar_ab[:, :, :, b], (nmob, 1, 1, 1))),
        )
...
def second_BCH(
    mp, fock_hfa, fock_hfb, tmp1_aa, tmp1_bb, tmp1_ab, tmp1_ba, tmp1_bar_aa, tmp1_bar_bb, tmp1_bar_ab, tmp1_bar_ba, c0
):
    nocca, noccb = mp.get_nocc()
    nmoa, nmob = mp.get_nmo()
    nvira, nvirb = nmoa - nocca, nmob - noccb
    ...
    y1_a = numpy.einsum(
        'ijkl -> kl',
        numpy.einsum('ijkl -> klij', numpy.tile(fock_hfa[:nocca, nocca:], (nocca, nvira, 1, 1))) * tmp1_bar_aa,
    )

    y1_a += numpy.einsum(
        'ijkl -> kl',
        numpy.einsum('ijkl -> klij', numpy.tile(fock_hfb[:noccb, noccb:], (nocca, nvira, 1, 1))) * tmp1_bar_ba,
    )

    c1_a[:nocca, nocca:] += 1.0 * numpy.einsum('ijkl -> ij', numpy.tile(y1_a, (nocca, nvira, 1, 1)) * tmp1_bar_aa)

    c1_b[:noccb, noccb:] += 1.0 * numpy.einsum('ijkl -> ij', numpy.tile(y1_a, (noccb, nvirb, 1, 1)) * tmp1_bar_ba)

    y1_b = numpy.einsum(
        'ijkl -> kl',
        numpy.einsum('ijkl -> klij', numpy.tile(fock_hfb[:noccb, noccb:], (noccb, nvirb, 1, 1))) * tmp1_bar_bb,
    )

    y1_b += numpy.einsum(
        'ijkl -> kl',
        numpy.einsum('ijkl -> klij', numpy.tile(fock_hfa[:nocca, nocca:], (noccb, nvirb, 1, 1))) * tmp1_bar_ab,
    )

    c1_a[:nocca, nocca:] += 1.0 * numpy.einsum('ijkl -> ij', numpy.tile(y1_b, (nocca, nvira, 1, 1)) * tmp1_bar_ab)
    c1_b[:noccb, noccb:] += 1.0 * numpy.einsum('ijkl -> ij', numpy.tile(y1_b, (noccb, nvirb, 1, 1)) * tmp1_bar_bb)
    ...
    for c in range(nvira):
        y1_aa += numpy.einsum(
            'ijkl -> klij', numpy.tile(fock_hfa[nocca:, c - nvira].T, (nocca, nvira, nocca, 1))
        ) * numpy.einsum('ijkl ->jikl', numpy.tile(tmp1_bar_aa[:, c, :, :], (nvira, 1, 1, 1)))
        y1_ab += numpy.einsum(
            'ijkl -> ilkj', numpy.tile(fock_hfa[nocca:, c - nvira].T, (nocca, nvirb, noccb, 1))
        ) * numpy.einsum('ijkl ->jikl', numpy.tile(tmp1_bar_ab[:, c, :, :], (nvira, 1, 1, 1)))

    for c in range(nvirb):
        y1_ba += numpy.einsum(
            'ijkl -> ilkj', numpy.tile(fock_hfb[noccb:, c - nvirb].T, (noccb, nvira, nocca, 1))
        ) * numpy.einsum('ijkl ->jikl', numpy.tile(tmp1_bar_ba[:, c, :, :], (nvirb, 1, 1, 1)))
        y1_bb += numpy.einsum(
            'ijkl -> klij', numpy.tile(fock_hfb[noccb:, c - nvirb].T, (noccb, nvirb, noccb, 1))
        ) * numpy.einsum('ijkl ->jikl', numpy.tile(tmp1_bar_bb[:, c, :, :], (nvirb, 1, 1, 1)))

    for k in range(nocca):
        c1_a[:nocca, k] += 1.0 * numpy.einsum(
            'ijkl -> k', tmp1_aa * numpy.einsum('ijkl -> jkil', numpy.tile(y1_aa[:, :, k, :], (nocca, 1, 1, 1)))
        )
        c1_a[:nocca, k] += 1.0 * numpy.einsum(
            'ijkl -> k', tmp1_ba * numpy.einsum('ijkl -> jkil', numpy.tile(y1_ba[:, :, k, :], (nocca, 1, 1, 1)))
        )

    for k in range(noccb):
        c1_b[:noccb, k] += 1.0 * numpy.einsum(
            'ijkl -> k', tmp1_bb * numpy.einsum('ijkl -> jkil', numpy.tile(y1_bb[:, :, k, :], (noccb, 1, 1, 1)))
        )
        c1_b[:noccb, k] += 1.0 * numpy.einsum(
            'ijkl -> k', tmp1_ab * numpy.einsum('ijkl -> jkil', numpy.tile(y1_ab[:, :, k, :], (noccb, 1, 1, 1)))
        )

    c0 -= 1.0 * numpy.sum(tmp1_aa * y1_aa) + 1.0 * numpy.sum(tmp1_bb * y1_bb)
    c0 -= 1.0 * numpy.sum(tmp1_ab * y1_ab) + 1.0 * numpy.sum(tmp1_ba * y1_ba)
    ...
    for k in range(nocca):
        c1_a[:nocca, k] += 1.0 * numpy.einsum('ijkl -> i', tmp1_aa * numpy.tile(y1_aa[k, :, :, :], (nocca, 1, 1, 1)))
        c1_a[:nocca, k] += 1.0 * numpy.einsum('ijkl -> i', tmp1_ab * numpy.tile(y1_ab[k, :, :, :], (nocca, 1, 1, 1)))
    ...
    for k in range(noccb):
        c1_b[:noccb, k] += 1.0 * numpy.einsum('ijkl -> i', tmp1_bb * numpy.tile(y1_bb[k, :, :, :], (noccb, 1, 1, 1)))
    for k in range(noccb):
        if mp.break_sym:
            c1_b[:noccb, k] += 1.0 * numpy.einsum(
                'ijkl -> i', tmp1_ba * numpy.einsum('ijkl -> jkil', numpy.tile(y1_ba[:, :, k, :], (nocca, 1, 1, 1)))
            )
        else:
            c1_b[:noccb, k] += 1.0 * numpy.einsum(
                'ijkl -> i', tmp1_ba * numpy.tile(y1_ba[k, :, :, :], (noccb, 1, 1, 1))
            )
    ...
    for k in range(nocca):
        y1_aa += numpy.einsum('ijkl -> ljik', numpy.tile(fock_hfa[:nocca, k], (nocca, nvira, nvira, 1))) * numpy.tile(
            tmp1_bar_aa[k, :, :, :], (nocca, 1, 1, 1)
        )
        y1_ab += numpy.einsum('ijkl -> lkij', numpy.tile(fock_hfa[:nocca, k], (noccb, nvirb, nvira, 1))) * numpy.tile(
            tmp1_bar_ab[k, :, :, :], (nocca, 1, 1, 1)
        )

    for k in range(noccb):
        y1_ba += numpy.einsum('ijkl -> lkij', numpy.tile(fock_hfb[:noccb, k], (nocca, nvira, nvirb, 1))) * numpy.tile(
            tmp1_bar_ba[k, :, :, :], (noccb, 1, 1, 1)
        )
        y1_bb += numpy.einsum('ijkl -> ljik', numpy.tile(fock_hfb[:noccb, k], (noccb, nvirb, nvirb, 1))) * numpy.tile(
            tmp1_bar_bb[k, :, :, :], (noccb, 1, 1, 1)
        )
    ...
    for k in range(nocca):
        y1_a[:, k] += numpy.einsum('ijkl -> i', tmp1_aa * numpy.tile(tmp1_bar_aa[k, :, :, :], (nocca, 1, 1, 1)))
        y1_a[:, k] += numpy.einsum('ijkl -> i', tmp1_ab * numpy.tile(tmp1_bar_ab[k, :, :, :], (nocca, 1, 1, 1)))

    for k in range(nocca):
        c1_a[:, k] -= 1.0 * numpy.einsum('ij -> i', fock_hfa[:nocca, :].T * numpy.tile(y1_a[:, k], (nmoa, 1)))

    for k in range(noccb):
        y1_b[:, k] += numpy.einsum('ijkl -> i', tmp1_bb * numpy.tile(tmp1_bar_bb[k, :, :, :], (noccb, 1, 1, 1)))
        y1_b[:, k] += numpy.einsum('ijkl -> i', tmp1_ba * numpy.tile(tmp1_bar_ba[k, :, :, :], (noccb, 1, 1, 1)))

    for k in range(noccb):
        c1_b[:, k] -= 1.0 * numpy.einsum('ij -> i', fock_hfb[:noccb, :].T * numpy.tile(y1_b[:, k], (nmob, 1)))
    ...
    for c in range(nvira):
        c1_a[nocca:, c + nocca] += 1.0 * numpy.einsum(
            'ijkl -> l', tmp1_aa * numpy.einsum('ijkl -> jkli', numpy.tile(y1_aa[:, :, :, c], (nvira, 1, 1, 1)))
        )
        c1_a[nocca:, c + nocca] += 1.0 * numpy.einsum(
            'ijkl -> l', tmp1_ba * numpy.einsum('ijkl -> jkli', numpy.tile(y1_ba[:, :, :, c], (nvira, 1, 1, 1)))
        )

    for c in range(nvirb):
        c1_b[noccb:, c + noccb] += 1.0 * numpy.einsum(
            'ijkl -> l', tmp1_bb * numpy.einsum('ijkl -> jkli', numpy.tile(y1_bb[:, :, :, c], (nvirb, 1, 1, 1)))
        )
        c1_b[noccb:, c + noccb] += 1.0 * numpy.einsum(
            'ijkl -> l', tmp1_ab * numpy.einsum('ijkl -> jkli', numpy.tile(y1_ab[:, :, :, c], (nvirb, 1, 1, 1)))
        )
    ...
    for c in range(nvira):
        c1_a[nocca:, c + nocca] += 1.0 * numpy.einsum(
            'ijkl -> j', tmp1_aa * numpy.einsum('ijkl -> jikl', numpy.tile(y1_aa[:, c, :, :], (nvira, 1, 1, 1)))
        )
        c1_a[nocca:, c + nocca] += 1.0 * numpy.einsum(
            'ijkl -> j', tmp1_ab * numpy.einsum('ijkl -> jikl', numpy.tile(y1_ab[:, c, :, :], (nvira, 1, 1, 1)))
        )
    ...
    for c in range(nvirb):
        c1_b[noccb:, c + noccb] += 1.0 * numpy.einsum(
            'ijkl -> j', tmp1_bb * numpy.einsum('ijkl -> jikl', numpy.tile(y1_bb[:, c, :, :], (nvirb, 1, 1, 1)))
        )
        c1_b[noccb:, c + noccb] += 1.0 * numpy.einsum(
            'ijkl -> j', tmp1_ba * numpy.einsum('ijkl -> jikl', numpy.tile(y1_ba[:, c, :, :], (nvirb, 1, 1, 1)))
        )
    ...
    for c in range(nvira):
        y1_a[:, c] += numpy.einsum(
            'ijkl -> j', tmp1_aa * numpy.einsum('ijkl -> jikl', numpy.tile(tmp1_bar_aa[:, c, :, :], (nvira, 1, 1, 1)))
        )
        y1_a[:, c] += numpy.einsum(
            'ijkl -> j', tmp1_ab * numpy.einsum('ijkl -> jikl', numpy.tile(tmp1_bar_ab[:, c, :, :], (nvira, 1, 1, 1)))
        )

    for c in range(nvira):
        c1_a[:, c + nocca] -= 1.0 * numpy.einsum('ij -> i', fock_hfa[nocca:, :].T * numpy.tile(y1_a[:, c], (nmoa, 1)))

    for c in range(nvirb):
        y1_b[:, c] += numpy.einsum(
            'ijkl -> j', tmp1_bb * numpy.einsum('ijkl -> jikl', numpy.tile(tmp1_bar_bb[:, c, :, :], (nvirb, 1, 1, 1)))
        )
        y1_b[:, c] += numpy.einsum(
            'ijkl -> j', tmp1_ba * numpy.einsum('ijkl -> jikl', numpy.tile(tmp1_bar_ba[:, c, :, :], (nvirb, 1, 1, 1)))
        )

    for c in range(nvirb):
        c1_b[:, c + noccb] -= 1.0 * numpy.einsum('ij -> i', fock_hfb[noccb:, :].T * numpy.tile(y1_b[:, c], (nmob, 1)))
...
def mom_select(mp, mo_coeff_init, mo_coeff_new):
    ...
    ia, ib = mp.occ_exc
    aa, ab = mp.vir_exc
    Oa = numpy.matmul(mo_coeff_init[0][:, 0:nocca].T, numpy.matmul(ovi, mo_coeff_new[0][:, :]))
    # print("Oa")
    # print(Oa)
    Pa = []
    for j in range(nmoa):
    ...
    for j in range(nmoa):
        if Pa[j] == max_el:
            indxa = j
    if (ib is not None) and (ab is not None):
        Ob = numpy.matmul(mo_coeff_init[1][:, 0:noccb].T, numpy.matmul(ovi, mo_coeff_new[1][:, :]))
        Pb = []
        for j in range(nmob):
            tmp = 0.0
            for i in range(noccb):
...
def make_rdm1(mp, use_t2=False, use_ao=True, **kwargs):
    ...
    if not use_t2:
        ...
        if use_ao:
            rdm1_ao = (
                reduce(numpy.dot, (mo_coeff[0], rdm1[0], mo_coeff[0].T)),
                reduce(numpy.dot, (mo_coeff[1], rdm1[1], mo_coeff[1].T)),
            )
            return rdm1_ao
        else:
...
def _gamma1_intermediates(mp, t2):
    t2aa, t2ab, t2bb = t2
    dooa = lib.einsum('imef,jmef->ij', t2aa.conj(), t2aa) * -0.5
    dooa -= lib.einsum('imef,jmef->ij', t2ab.conj(), t2ab)
    doob = lib.einsum('imef,jmef->ij', t2bb.conj(), t2bb) * -0.5
    doob -= lib.einsum('mief,mjef->ij', t2ab.conj(), t2ab)

    dvva = lib.einsum('mnae,mnbe->ba', t2aa.conj(), t2aa) * 0.5
    dvva += lib.einsum('mnae,mnbe->ba', t2ab.conj(), t2ab)
    dvvb = lib.einsum('mnae,mnbe->ba', t2bb.conj(), t2bb) * 0.5
