...
def kernel(mp, mo_energy=None, mo_coeff=None, eris=None, with_t2=WITH_T2, verbose=logger.NOTE):
    ...
    for it in range(niter):
        ...
        # Build DIIS Residual
        diis_r_a = A.dot(err_a_mo + err_b_mo + 30 * err_ab_mo + 30 * err_ba_mo).dot(A)
        ...
        # Build DIIS Residual
        diis_r_b = A.dot(err_a_mo + err_b_mo + 30 * err_ab_mo + 30 * err_ba_mo).dot(A)
    ...
    if conv:
        ...
        print('HF has not converged')

    print('HF energy = ', ene_tot)

    #################### MP2

    co_a = numpy.asarray(mp.mo_coeff[0][:, :nocca], order='F')
    cv_a = numpy.asarray(mp.mo_coeff[0][:, nocca:], order='F')
    co_b = numpy.asarray(mp.mo_coeff[1][:, :noccb], order='F')
    cv_b = numpy.asarray(mp.mo_coeff[1][:, noccb:], order='F')
    ...
    eia_a = mp.mo_energy[0][:nocca, None] - mp.mo_energy[0][None, nocca:]
    eia_b = mp.mo_energy[1][:noccb, None] - mp.mo_energy[1][None, noccb:]
    ...
    for i in range(nocca):
        ...
        emp2 += numpy.einsum('jab,jab', t2i, eris_ovov) * 0.5
        emp2 -= numpy.einsum('jab,jba', t2i, eris_ovov) * 0.5
...
def first_BCH(mp, fock_hfa, fock_hfb, tmp1_bar, c0):
    ...
    c1_a[:, :nocca] += 2.0 * lib.einsum('ijpl,ijkl -> pk', h2mo_aa_ovgv, tmp1_bar_aa)
    c1_a[:, :nocca] += 2.0 * lib.einsum('ijpl,ijkl -> pk', h2mo_ba_ovgv, tmp1_bar_ba)

    c1_a[:, nocca:nmoa] -= 2.0 * lib.einsum('ijkp,ijkl -> pl', h2mo_aa_ovog, tmp1_bar_aa)
    c1_a[:, nocca:nmoa] -= 2.0 * lib.einsum('ijkp,ijkl -> pl', h2mo_ba_ovog, tmp1_bar_ba)
    ...
    c1_b[:, :noccb] += 2.0 * lib.einsum('ijpl,ijkl -> pk', h2mo_bb_ovgv, tmp1_bar_bb)
    c1_b[:, :noccb] += 2.0 * lib.einsum('ijpl,ijkl -> pk', h2mo_ab_ovgv, tmp1_bar_ab)

    c1_b[:, noccb:nmob] -= 2.0 * lib.einsum('ijkp,ijkl -> pl', h2mo_bb_ovog, tmp1_bar_bb)
    c1_b[:, noccb:nmob] -= 2.0 * lib.einsum('ijkp,ijkl -> pl', h2mo_ab_ovog, tmp1_bar_ab)
...
def make_IPEA(mp):
    ...
    ea_obmp2 = eV * (-mo_energy[0][L] - 1.0 * tmp2)
