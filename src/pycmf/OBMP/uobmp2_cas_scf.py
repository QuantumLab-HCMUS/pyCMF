...
def kernel(mp, mo_energy=None, mo_coeff=None, eris=None, with_t2=WITH_T2, verbose=logger.NOTE):
    ...
    for it in range(niter):
        ...
        cg = numpy.array(
            [
                numpy.asarray(mo_coeff[0][:, nocc_inact[0] : nocc_inact[0] + nact[0]], order='F'),
                numpy.asarray(mo_coeff[1][:, nocc_inact[1] : nocc_inact[1] + nact[1]], order='F'),
            ]
        )
        ...
        mo_coeff_a = numpy.matmul(mo_coeff[0][:, nocc_inact[0] : nocc_inact[0] + nact[0]], U)

        mo_energy_b, U = scipy.linalg.eigh(fock[1])
        mo_coeff_b = numpy.matmul(mo_coeff[1][:, nocc_inact[1] : nocc_inact[1] + nact[1]], U)
