...
def kernel(mp, mo_energy=None, mo_coeff=None, eris=None, with_t2=WITH_T2, verbose=logger.NOTE):
    ...
    print('initialization time', time_initial - t0)
    ...
    for it in range(niter):
        ...
        # t_HF = log.timer('the HF part', *t0)
        t_Hatree = time.time()
        print('HF time', t_Hatree - t0)

        ####################
        #### MP1 amplitude
        t0 = time.time()
        qov_a, qov_b = make_amp(mp)
        # t_eri = log.timer('making amplitude', *t_HF)
        t_eri = time.time()
        print('HF time', t_eri - t0)
        ...
        # t1 = log.timer('making first BCH', *t_eri)
        t_st = time.time()
        print('1st BCH time', t_st - t0)
        ...
        if mp.second_order:
            ...
            # t2 = log.timer('making second BCH', *t1)
            t_nd = time.time()
            print('1st BCH time', t_nd - t0)
    ...
    print()
    if conv:
        print('DFUOB-MP2 has converged')
    else:
        print('DFUOB-MP2 has not converged')
...
def make_S2(mp, qov_a, qov_b):
    ...
    for i in range(nocca):
        ...
        t2i_ab = (gi_ab / lib.direct_sum('jb+a->jab', eia_b, eia_a[i])).transpose(1, 0, 2) * mp.ampf
        t2i_bar_ab = t2i_ab
        tmp += lib.einsum('a,ajb -> jb', Sja_BA[i, :], t2i_bar_ab)
...
def make_amp(mp):
    ...
    print('amplitude memory: %.1f MiB' % current_memory()[0])
...
def first_BCH(mp, fock_hfa, fock_hfb, qov_a, qov_b, c0):
    ...
    for i in range(nocca):
        ...
        c0 -= 1 * numpy.trace(
            numpy.dot(buf_aa.reshape(nvira, nocca * nvira), t2i_bar_aa.reshape(nvira, nocca * nvira).T)
        )  # [4]
        ...
        c1_a[:, nocca:nmoa] -= 2.0 * numpy.dot(qog_a.T, Qb)  # [3]
        c1_a[:nocca, nocca:] += 2.0 * numpy.dot(
            t2i_bar_aa.reshape(nvira, nocca * nvira).T, fock_hfa[i, nocca:]
        ).reshape(nocca, nvira)  # [1]
        ...
        c0 -= 2 * numpy.trace(
            numpy.dot(buf_ab.reshape(nvira, noccb * nvirb), t2i_bar_ab.reshape(nvira, noccb * nvirb).T)
        )  # [AB=BA]#[4]
        Qb = numpy.dot(qov_b, t2i_bar_ab.reshape(nvira, noccb * nvirb).T)

        c1_a[:, nocca:nmoa] -= 2.0 * numpy.dot(qog_a.T, Qb)  # [3]
        c1_b[:noccb, noccb:] += 2.0 * numpy.dot(
            t2i_bar_ab.reshape(nvira, noccb * nvirb).T, fock_hfa[i, nocca:]
        ).reshape(noccb, nvirb)  # [1]

        # [BA]
        c1_a[i, nocca:] += 2.0 * lib.einsum('bIA, IA -> b', t2i_bar_ab, fock_hfb[:noccb, noccb:])  # [1]

    t1 = time.time()
    print('part1 of 1st BCH time', t1 - t0)
    ...
    for b in range(1, nvirb):
        Qov_b = numpy.concatenate((Qov_b, qov_b.reshape(naux, noccb, nvirb)[:, :, b]), axis=1)
    ...
    for a in range(nvira):
        ...
        t2a_aa = css * ga_aa / lib.direct_sum('bj+i->bji', eia_a.T, eia_a.T[a]) * mp.ampf
        t2a_aa = t2a_aa.transpose(1, 0, 2)
        ...
        c1_a[:, 0:nocca] += 2.0 * numpy.dot(qgv_a.T, Qj)  # [2]
        ...
        t2a_ab = cos * ga_ab / lib.direct_sum('BJ+i->BJi', eia_b.T, eia_a.T[a]) * mp.ampf
        t2a_ab = t2a_ab.transpose(1, 0, 2)
        t2a_bar_ab = t2a_ab

        Qj = numpy.dot(qov_b, t2a_bar_ab.transpose(2, 1, 0).reshape(noccb * nvirb, nocca))
        c1_a[:, 0:nocca] += 2.0 * numpy.dot(qgv_a.T, Qj)  # [2]

    t2 = time.time()
    print('part2 of 1st BCH time', t2 - t1)

    for b in range(nvirb):
        # [BB]

        buf_bb = numpy.dot(qov_b.reshape(naux, noccb, nvirb)[:, :, b].T, Qov_b).reshape(noccb, nvirb, noccb)
        ga_bb = numpy.array(buf_bb, copy=False)
        ga_bb = ga_bb.reshape(noccb, nvirb, noccb).transpose(1, 0, 2)
        t2a_bb = css * ga_bb / lib.direct_sum('BJ+I->BJI', eia_b.T, eia_b.T[b]) * mp.ampf
        t2a_bb = t2a_bb.transpose(1, 0, 2)
        ...
        c1_b[:, 0:noccb] += 2.0 * numpy.dot(qgv_b.T, Qj)  # [2]

        # [BA]

        buf_ba = numpy.dot(qov_b.reshape(naux, noccb, nvirb)[:, :, b].T, Qov_a).reshape(noccb, nvira, nocca)
        ga_ba = numpy.array(buf_ba, copy=False)
        ga_ba = ga_ba.reshape(noccb, nvira, nocca).transpose(1, 0, 2)
        t2a_ba = cos * ga_ba / lib.direct_sum('bj+I->bjI', eia_a.T, eia_b.T[b]) * mp.ampf
        t2a_ba = t2a_ba.transpose(1, 0, 2)
        t2a_bar_ba = t2a_ba

        Qj = numpy.dot(qov_a, t2a_bar_ba.transpose(2, 1, 0).reshape(nocca * nvira, noccb))
        c1_b[:, 0:noccb] += 2.0 * numpy.dot(qgv_b.T, Qj)  # [2]

    t3 = time.time()
    print('part3 of 1st BCH time', t3 - t2)
    for i in range(noccb):
        # [BB]
        ...
        c0 -= numpy.trace(
            numpy.dot(buf_bb.reshape(nvirb, noccb * nvirb), t2i_bar_bb.reshape(nvirb, noccb * nvirb).T)
        )  # [4]
        ...
        c1_b[:, noccb:nmob] -= 2.0 * numpy.dot(qog_b.T, Qb)  # [3]
        c1_b[:noccb, noccb:] += 2.0 * numpy.dot(
            t2i_bar_bb.reshape(nvirb, noccb * nvirb).T, fock_hfb[i, noccb:]
        ).reshape(noccb, nvirb)  # [1]
        ...
        Qb = numpy.dot(qov_a, t2i_bar_ba.reshape(nvirb, nocca * nvira).T)
        c1_b[:, noccb:nmob] -= 2.0 * numpy.dot(qog_b.T, Qb)  # [3]

    t4 = time.time()
    print('part3 of 1st BCH time', t4 - t3)
...
def second_BCH(mp, fock_a, fock_b, fock_hfa, fock_hfb, qov_a, qov_b, c0):
    ...
    for i in range(nocca):
        ...
        t2i_aa = css * (gi_aa / lib.direct_sum('jb+a->jba', eia_a, eia_a[i])).transpose(1, 0, 2)
        t2i_bar_aa = t2i_aa - t2i_aa.T
        ...
        gi_ab = numpy.array(buf_ab, copy=False)
        gi_ab = gi_ab.reshape(nvira, noccb, nvirb).transpose(1, 0, 2)
        t2i_ab = cos * (gi_ab / lib.direct_sum('jb+a->jba', eia_b, eia_a[i])).transpose(1, 0, 2)
        t2i_bar_ab = t2i_ab
    ...
    t1 = time.time()
    print('part1 of 2nd BCH time', t1 - t0)
    for i in range(noccb):
        Qov_b = qov_b[:, i * nvirb : (i + 1) * nvirb]
        buf_bb = numpy.dot(Qov_b.T, qov_b).reshape(nvirb, noccb, nvirb)
        gi_bb = numpy.array(buf_bb, copy=False)
        gi_bb = gi_bb.reshape(nvirb, noccb, nvirb).transpose(1, 0, 2)
        t2i_bb = css * (gi_bb / lib.direct_sum('jb+a->jba', eia_b, eia_b[i])).transpose(1, 0, 2)
        t2i_bar_bb = t2i_bb - t2i_bb.T
        ...
        t2i_ba = cos * (gi_ba / lib.direct_sum('jb+a->jba', eia_a, eia_b[i])).transpose(1, 0, 2)
        t2i_bar_ba = t2i_ba
    ...
    t2 = time.time()
    print('part2 of 2nd BCH time', t2 - t1)
    Qov_a = qov_a.reshape(naux, nocca, nvira)[:, :, 0]
    for a in range(1, nvira):
        Qov_a = numpy.concatenate((Qov_a, qov_a.reshape(naux, nocca, nvira)[:, :, a]), axis=1)
    ...
    for a in range(nvira):
        ...
        ga_ab = numpy.array(buf_ab, copy=False)
        ga_ab = ga_ab.reshape(nocca, nvirb, noccb).transpose(1, 0, 2)
        t2a_ab = cos * ga_ab / lib.direct_sum('bj+i->bji', eia_b.T, eia_a.T[a])
        # [4]
    ...
    t3 = time.time()
    print('part3 of 2nd BCH time', t3 - t2)
    for a in range(nvirb):
        ...
        ga_ba = numpy.array(buf_ba, copy=False)
        ga_ba = ga_ba.reshape(noccb, nvira, nocca).transpose(1, 0, 2)
        t2a_ba = cos * ga_ba / lib.direct_sum('bj+i->bji', eia_a.T, eia_b.T[a])
        # [4]
    ...
    t4 = time.time()
    print('part4 of 2nd BCH time', t4 - t3)
...
def make_IPEA(mp):
    ...
    for h in range(nocca):
        x_a = (
            mo_energy[0][:nocca, None, None]
            + mo_energy[0][None, None, :nocca]
            - mo_energy[0][None, nocca:, None]
            - mo_energy[0][None, h, None]
            - mp.shift
        )
        x_b = (
            mo_energy[1][:noccb, None, None]
            + mo_energy[0][None, None, :nocca]
            - mo_energy[1][None, noccb:, None]
            - mo_energy[0][None, h, None]
            - mp.shift
        )
        ...
        ip_obmp2 = eV * (-mo_energy[0][h] + 1.0 * tmp2)
    ...
    x_b = (
        mo_energy[1][:noccb, None, None]
        + mo_energy[1][None, L, None]
        - mo_energy[1][None, noccb:, None]
        - mo_energy[1][None, None, noccb:]
        - mp.shift
    )
...
def make_rdm2(mp, t2=None):
    ...
    if not (mp.frozen == 0 or mp.frozen is None):
        ...
        dm2aa[vidxa[:, None, None, None], oidxa[:, None, None], vidxa[:, None], oidxa] = tmp.conj().transpose(
            1, 0, 3, 2
        )

        tmp = t2bb.transpose(0, 2, 1, 3)
        dm2bb[oidxb[:, None, None, None], vidxb[:, None, None], oidxb[:, None], vidxb] = tmp
        dm2bb[vidxb[:, None, None, None], oidxb[:, None, None], vidxb[:, None], oidxb] = tmp.conj().transpose(
            1, 0, 3, 2
        )

        dm2ab[oidxa[:, None, None, None], vidxa[:, None, None], oidxb[:, None], vidxb] = t2ab.transpose(0, 2, 1, 3)
        dm2ab[vidxa[:, None, None, None], oidxa[:, None, None], vidxb[:, None], oidxb] = t2ab.conj().transpose(
            2, 0, 3, 1
        )
...
def make_fc(mp, dm0, it=None, R_reslv=None, hfc_nuc=None, verbose=None):
    ...
    for i, atm_id in enumerate(hfc_nuc):
        ...
        logger.info(mp, 'FC of atom %d : %8.6f (in MHz)', atm_id, (2.0 * fac * nuc_gyro * fc))
...
def mom_occ_(mp, occorb, setocc):
    ...
    def get_occ(mo_energy=None, mo_coeff=None):
        ...
        if int(numpy.sum(mo_occ[0])) != nocc_a:
            log.error('mom alpha electron occupation numbers do not match: %d, %d', nocc_a, int(numpy.sum(mo_occ[0])))
        if int(numpy.sum(mo_occ[1])) != nocc_b:
            log.error('mom alpha electron occupation numbers do not match: %d, %d', nocc_b, int(numpy.sum(mo_occ[1])))
...
def _ao2mo_ovov(mp, orbs, feri, max_memory=2000, verbose=None):
    ...
    ftmp = lib.H5TmpFile()
    disk = (
        nocca**2.0 * (nao * (nao + dmax) / 2 + nvira**2)
        + noccb**2.0 * (nao * (nao + dmax) / 2 + nvirb**2)
    ...
    occblk = int(min(max(nocca, noccb), max(4, 250 / nocca, max_memory * 0.9e6 / 8 / (nao**2.0 * nocca) / 5)))
...
if __name__ == '__main__':
    ...
    mol = gto.Mole()
    mol.atom = [[6, (0.0, 0.0, 0.6)], [8, (0.0, 0.0, 0.0)]]
