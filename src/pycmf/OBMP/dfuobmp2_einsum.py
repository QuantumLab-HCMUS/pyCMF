...
def first_BCH(mp, fock_hfa, fock_hfb, qov_a, qov_b, c0):
    ...
    for i in range(nocca):
        ...
        c1_a[:, 0:nocca] += 2.0 * lib.einsum(
            'apb, ajb -> pj', numpy.dot(Qov_a.T, qgv_a).reshape(nvira, nmoa, nvira), t2i_bar_aa
        )
        c0 -= 1.0 * lib.einsum('ajb, ajb -> ', buf_aa, t2i_bar_aa)
        c0 -= 1.0 * lib.einsum('ajb, ajb -> ', numpy.dot(Qov_a.T, qov_b).reshape(nvira, noccb, nvirb), t2i_bar_ab)
        c1_b[:, 0:noccb] += 2.0 * lib.einsum(
            'apb, ajb -> pj', numpy.dot(Qov_a.T, qgv_b).reshape(nvira, nmob, nvirb), t2i_bar_ab
        )
        c1_a[:, nocca:nmoa] -= 2.0 * lib.einsum(
            'ajp, ajb -> pb', numpy.dot(Qov_a.T, qog_a).reshape(nvira, nocca, nmoa), t2i_bar_aa
        )
        c1_b[:, noccb:nmob] -= 2.0 * lib.einsum(
            'ajp, ajb -> pb', numpy.dot(Qov_a.T, qog_b).reshape(nvira, noccb, nmob), t2i_bar_ab
        )
        c1_a[:nocca, nocca:] += 2.0 * lib.einsum('ajb,a -> jb', t2i_bar_aa, fock_hfa[i, nocca:])
        c1_b[:noccb, noccb:] += 2.0 * lib.einsum('ajb,a -> jb', t2i_bar_ab, fock_hfa[i, nocca:])
    ...
    for i in range(noccb):
        ...
        c1_a[:, 0:nocca] += 2.0 * lib.einsum(
            'apb, ajb -> pj', numpy.dot(Qov_b.T, qgv_a).reshape(nvirb, nmoa, nvira), t2i_ba
        )
        c1_b[:, 0:noccb] += 2.0 * lib.einsum(
            'apb, ajb -> pj', numpy.dot(Qov_b.T, qgv_b).reshape(nvirb, nmob, nvirb), t2i_bar_bb
        )
        c0 -= 1.0 * lib.einsum('ajb, ajb -> ', numpy.dot(Qov_b.T, qov_a).reshape(nvirb, nocca, nvira), t2i_ba)
        c0 -= 1.0 * lib.einsum('ajb, ajb -> ', numpy.dot(Qov_b.T, qov_b).reshape(nvirb, noccb, nvirb), t2i_bar_bb)
        c1_a[:, nocca:nmoa] -= 2.0 * lib.einsum(
            'ajp, ajb -> pb', numpy.dot(Qov_b.T, qog_a).reshape(nvirb, nocca, nmoa), t2i_ba
        )
        c1_b[:, noccb:nmob] -= 2.0 * lib.einsum(
            'ajp, ajb -> pb', numpy.dot(Qov_b.T, qog_b).reshape(nvirb, noccb, nmob), t2i_bar_bb
        )
        c1_b[:noccb, noccb:] += 2.0 * lib.einsum('ajb,a -> jb', t2i_bar_bb, fock_hfb[i, noccb:])
        c1_a[:nocca, nocca:] += 2.0 * lib.einsum('ajb,a -> jb', t2i_bar_ba, fock_hfb[i, noccb:])
