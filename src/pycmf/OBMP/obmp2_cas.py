...
def kernel(mp, mo_energy=None, mo_coeff=None, eris=None, with_t2=WITH_T2, verbose=logger.NOTE):
    ...
    for it in range(niter):
        ...
        mo_coeff_act = numpy.matmul(mp.mo_coeff[:, nocc_inact : nocc_inact + nact], U)
...
def make_veff_core(mp):
    ...
    veff_core += 2.0 * numpy.einsum('ijkk -> ij', h2mo_ggcc) - numpy.einsum('ijjk -> ik', h2mo_gccg)
...
def make_veff(mp):
    ...
    veff += 2.0 * numpy.einsum('ijkk -> ij', h2mo_ggoo_act) - numpy.einsum('ijjk -> ik', h2mo_goog_act)

    c0_hf = 0.0
    for i in range(nocc_act):
        for j in range(nocc_act):
            c0_hf -= 2.0 * h2mo_ggoo_act[i, i, j, j] - h2mo_ggoo_act[i, j, j, i]
...
def first_BCH(mp, fock_hf, tmp1, tmp1_bar, c0):
    ...
    c0 -= 4.0 * numpy.sum(h2mo_ovov_act * tmp1_bar)
    ####################### c1[j,B] #########################

    c1_jb = 4.0 * numpy.einsum(
        'ijkl -> ij',
        numpy.einsum('ijkl -> klij', tmp1_bar) * numpy.tile(fock_hf[:nocc_act, nocc_act:], (nocc_act, nvir_act, 1, 1)),
    )
    c1_jb = numpy.pad(c1_jb, [(0, nvir_act), (nocc_act, 0)], mode='constant')
    ####################### c1[p,j] #########################
    for j in range(nocc_act):
        c1[:, j] = 4 * numpy.einsum(
            'ijkl -> k', h2mo_ovgv_act * numpy.einsum('ijkl -> jkil', numpy.tile(tmp1_bar[:, :, j, :], (nact, 1, 1, 1)))
        )

    ####################### c1[p,B] #########################
    for b in range(nvir_act):
        c1[:, b + nocc_act] = -4 * numpy.einsum(
            'ijkl -> l', h2mo_ovog_act * numpy.einsum('ijkl -> jkli', numpy.tile(tmp1_bar[:, :, :, b], (nact, 1, 1, 1)))
        )
...
def second_BCH(mp, fock_hf, tmp1, tmp1_bar, c0):
    ...
    y1 = numpy.einsum(
        'ijkl -> kl',
        numpy.einsum('ijkl -> klij', numpy.tile(fock_hf[:nocc_act, nocc_act:], (nocc_act, nvir_act, 1, 1))) * tmp1_bar,
    )

    c1[:nocc_act, nocc_act:] += 4.0 * numpy.einsum('ijkl -> ij', numpy.tile(y1, (nocc_act, nvir_act, 1, 1)) * tmp1_bar)
    ...
    for c in range(nvir_act):
        y1 += numpy.einsum(
            'ijkl -> klij', numpy.tile(fock_hf[nocc_act:, c - nvir_act].T, (nocc_act, nvir_act, nocc_act, 1))
        ) * numpy.einsum('ijkl ->jikl', numpy.tile(tmp1_bar[:, c, :, :], (nvir_act, 1, 1, 1)))

    for k in range(nocc_act):
        c1[:nocc_act, k] += 2.0 * (
            numpy.einsum(
                'ijkl -> k', tmp1 * numpy.einsum('ijkl -> jkil', numpy.tile(y1[:, :, k, :], (nocc_act, 1, 1, 1)))
            )
            + numpy.einsum('ijkl -> i', tmp1 * numpy.tile(y1[k, :, :, :], (nocc_act, 1, 1, 1)))
        )

    for b in range(nvir_act):
        c1[b + nocc_act, nocc_act:] -= 2.0 * numpy.einsum(
            'ijkl -> l', y1 * numpy.einsum('ijkl -> jkli', numpy.tile(tmp1[:, :, :, b], (nvir_act, 1, 1, 1)))
        )
    ...
    for k in range(nocc_act):
        y1 += numpy.einsum(
            'ijkl -> ljik', numpy.tile(fock_hf[:nocc_act, k], (nocc_act, nvir_act, nvir_act, 1))
        ) * numpy.tile(tmp1_bar[k, :, :, :], (nocc_act, 1, 1, 1))

    for c in range(nvir_act):
        c1[nocc_act:, c + nocc_act] += 2.0 * (
            numpy.einsum(
                'ijkl -> l', tmp1 * numpy.einsum('ijkl -> jkli', numpy.tile(y1[:, :, :, c], (nvir_act, 1, 1, 1)))
            )
            + numpy.einsum(
                'ijkl -> j', tmp1 * numpy.einsum('ijkl -> jikl', numpy.tile(y1[:, c, :, :], (nvir_act, 1, 1, 1)))
            )
        )
    ...
    for k in range(nocc_act):
        c1[:, k] -= 2.0 * numpy.einsum('ij -> i', fock_hf[:nocc_act, :].T * numpy.tile(y1[:, k], (nact, 1)))
    ...
    for c in range(nvir_act):
        y1[:, c] += numpy.einsum(
            'ijkl -> j', tmp1 * numpy.einsum('ijkl -> jikl', numpy.tile(tmp1_bar[:, c, :, :], (nvir_act, 1, 1, 1)))
        )

    for c in range(nvir_act):
        c1[:, c + nocc_act] -= 2.0 * numpy.einsum('ij -> i', fock_hf[nocc_act:, :].T * numpy.tile(y1[:, c], (nact, 1)))
...
class OBMP2(lib.StreamObject):
    ...
    def kernel(self, shift=0.0, mo_energy=None, mo_coeff=None, eris=None, with_t2=WITH_T2, _kern=kernel):
        ...
        self.e_corr, self.e_act, self.fock_hf, self.h1mo_act_vqe, self.h2mo_act, self.veff_core = _kern(
    ...
    def _finalize(self):
        ...
        logger.note(self, 'E(%s) = %.15g  E_corr = %.15g', self.__class__.__name__, self.e_act, self.e_corr)
        return self
