            ...
            for b in range(nvira):
                ...
                for i in range(nocca):
                    for a in range(nvira):
                        ...
                        c1_a[j, B] += 2.0 * fock_hfa[i, A] * tmp1_bar_aa[i, a, j, b]
                        for p in range(nmoa):
                            c1_a[p, j] += 2.0 * h2mo_aa[i, A, p, B] * tmp1_bar_aa[i, a, j, b]
                            c1_a[p, B] -= 2.0 * h2mo_aa[i, A, j, p] * tmp1_bar_aa[i, a, j, b]
                for i in range(noccb):
                    for a in range(nvirb):
                        A = a + noccb
                        c0 -= 1.0 * h2mo_ba[i, A, j, B] * tmp1_bar_ba[i, a, j, b]
                        c1_a[j, B] += 2.0 * fock_hfb[i, A] * tmp1_bar_ba[i, a, j, b]
                        for p in range(nmoa):
                            c1_a[p, j] += 2.0 * h2mo_ba[i, A, p, B] * tmp1_bar_ba[i, a, j, b]
                            c1_a[p, B] -= 2.0 * h2mo_ba[i, A, j, p] * tmp1_bar_ba[i, a, j, b]
            ...
            for b in range(nvirb):
                ...
                for i in range(noccb):
                    for a in range(nvirb):
                        ...
                        c1_b[j, B] += 2.0 * fock_hfb[i, A] * tmp1_bar_bb[i, a, j, b]
                        for p in range(nmob):
                            c1_b[p, j] += 2.0 * h2mo_bb[i, A, p, B] * tmp1_bar_bb[i, a, j, b]
                            c1_b[p, B] -= 2.0 * h2mo_bb[i, A, j, p] * tmp1_bar_bb[i, a, j, b]
                for i in range(nocca):
                    for a in range(nvira):
                        A = a + nocca
                        c0 -= 1.0 * h2mo_ab[i, A, j, B] * tmp1_bar_ab[i, a, j, b]
                        c1_b[j, B] += 2.0 * fock_hfa[i, A] * tmp1_bar_ab[i, a, j, b]
                        for p in range(nmoa):
                            c1_b[p, j] += 2.0 * h2mo_ab[i, A, p, B] * tmp1_bar_ab[i, a, j, b]
                            c1_b[p, B] -= 2.0 * h2mo_ab[i, A, j, p] * tmp1_bar_ab[i, a, j, b]
        # symmetrize c1
        for p in range(nmoa):
            for q in range(nmoa):
                        ...
                        for j in range(nocca):
                            for b in range(nvira):
                                if mp.break_sym:
                                    ...
                                    c1_b[i, k] += 1.0 * tmp1_ba[i, a, j, b] * y1_ba[k, a, j, b]
