    ...
    for it in range(niter):
            ...
            for a in range(nvira):
                for j in range(nocca):
                    for b in range(nvira):
                        ...
                        tmp1_aa[i, a, j, b] = 1.0 * h2mo_aa[i, A, j, B] / x
            ...
            for a in range(nvirb):
                for j in range(noccb):
                    for b in range(nvirb):
                        ...
                        tmp1_bb[i, a, j, b] = 1.0 * h2mo_bb[i, A, j, B] / x
            ...
            for a in range(nvira):
                for j in range(noccb):
                    for b in range(nvirb):
                        ...
                        tmp1_ab[i, a, j, b] = 1.0 * h2mo_ab[i, A, j, B] / x
            ...
            for a in range(nvirb):
                for j in range(nocca):
                    for b in range(nvira):
                        ...
                        tmp1_ba[i, a, j, b] = 1.0 * h2mo_ba[i, A, j, B] / x
        ...
        for j in range(nocca):
            for b in range(nvira):
                ...
                for i in range(nocca):
                    for a in range(nvira):
                        A = a + nocca
                        c0 -= 1.0 * h2mo_aa[i, A, j, B] * tmp1_bar_aa[i, a, j, b]
                        c1_a[j, B] += css * 2.0 * fock_hfa[i, A] * tmp1_bar_aa[i, a, j, b]
                        for p in range(nmoa):
                            c1_a[p, j] += css * 2.0 * h2mo_aa[i, A, p, B] * tmp1_bar_aa[i, a, j, b]
                            c1_a[p, B] -= css * 2.0 * h2mo_aa[i, A, j, p] * tmp1_bar_aa[i, a, j, b]
                for i in range(noccb):
                    for a in range(nvirb):
                        A = a + noccb
                        c0 -= 1.0 * h2mo_ba[i, A, j, B] * tmp1_bar_ba[i, a, j, b]
                        c1_a[j, B] += cos * 2.0 * fock_hfb[i, A] * tmp1_bar_ba[i, a, j, b]
                        for p in range(nmoa):
                            c1_a[p, j] += cos * 2.0 * h2mo_ba[i, A, p, B] * tmp1_bar_ba[i, a, j, b]
                            c1_a[p, B] -= cos * 2.0 * h2mo_ba[i, A, j, p] * tmp1_bar_ba[i, a, j, b]
        ...
        for j in range(noccb):
            for b in range(nvirb):
                ...
                for i in range(noccb):
                    for a in range(nvirb):
                        A = a + noccb
                        c0 -= 1.0 * h2mo_bb[i, A, j, B] * tmp1_bar_bb[i, a, j, b]
                        c1_b[j, B] += css * 2.0 * fock_hfb[i, A] * tmp1_bar_bb[i, a, j, b]
                        for p in range(nmob):
                            c1_b[p, j] += css * 2.0 * h2mo_bb[i, A, p, B] * tmp1_bar_bb[i, a, j, b]
                            c1_b[p, B] -= css * 2.0 * h2mo_bb[i, A, j, p] * tmp1_bar_bb[i, a, j, b]
                for i in range(nocca):
                    for a in range(nvira):
                        A = a + nocca
                        c0 -= 1.0 * h2mo_ab[i, A, j, B] * tmp1_bar_ab[i, a, j, b]
                        c1_b[j, B] += cos * 2.0 * fock_hfa[i, A] * tmp1_bar_ab[i, a, j, b]
                        for p in range(nmoa):
                            c1_b[p, j] += cos * 2.0 * h2mo_ab[i, A, p, B] * tmp1_bar_ab[i, a, j, b]
                            c1_b[p, B] -= cos * 2.0 * h2mo_ab[i, A, j, p] * tmp1_bar_ab[i, a, j, b]
        # symmetrize c1
        for p in range(nmoa):
            for q in range(nmoa):
        ...
        if mp.second_order:
            ...
            for i in range(nocca):
                for a in range(nvira):
                    for j in range(nocca):
                        for b in range(nvira):
                            A = a + nocca
                            c1_a[i, A] += 1.0 * y1_a[j, b] * tmp1_bar_aa[i, a, j, b]
            for i in range(noccb):
                for a in range(nvirb):
                    for j in range(nocca):
                        for b in range(nvira):
                            A = a + noccb
                            c1_b[i, A] += 1.0 * y1_a[j, b] * tmp1_bar_ba[i, a, j, b]
            ...
            for i in range(nocca):
                for a in range(nvira):
                    for j in range(noccb):
                        for b in range(nvirb):
                            A = a + nocca
                            c1_a[i, A] += 1.0 * y1_b[j, b] * tmp1_bar_ab[i, a, j, b]
            for i in range(noccb):
                for a in range(nvirb):
                    for j in range(noccb):
                        for b in range(nvirb):
                            A = a + noccb
                            c1_b[i, A] += 1.0 * y1_b[j, b] * tmp1_bar_bb[i, a, j, b]
            ...
            for j in range(nocca):
                for k in range(nocca):
                    for b in range(nvira):
                        for i in range(nocca):
                            for a in range(nvira):
                                c1_a[j, k] += 1.0 * tmp1_aa[i, a, j, b] * y1_aa[i, a, k, b]
                        for i in range(noccb):
                            for a in range(nvirb):
                                c1_a[j, k] += 1.0 * tmp1_ba[i, a, j, b] * y1_ba[i, a, k, b]
            ...
            for j in range(noccb):
                for k in range(noccb):
                    for b in range(nvirb):
                        for i in range(noccb):
                            for a in range(nvirb):
                                c1_b[j, k] += 1.0 * tmp1_bb[i, a, j, b] * y1_bb[i, a, k, b]
                        for i in range(nocca):
                            for a in range(nvira):
                                c1_b[j, k] += 1.0 * tmp1_ab[i, a, j, b] * y1_ab[i, a, k, b]

            for i in range(nocca):
                for a in range(nvira):
                    for j in range(nocca):
                        for b in range(nvira):
                            c0 -= 1.0 * tmp1_aa[i, a, j, b] * y1_aa[i, a, j, b]
                    for j in range(noccb):
                        for b in range(nvirb):
                            c0 -= 1.0 * tmp1_ab[i, a, j, b] * y1_ab[i, a, j, b]
            for i in range(noccb):
                for a in range(nvirb):
                    for j in range(noccb):
                        for b in range(nvirb):
                            c0 -= 1.0 * tmp1_bb[i, a, j, b] * y1_bb[i, a, j, b]
                    for j in range(nocca):
                        for b in range(nvira):
                            c0 -= 1.0 * tmp1_ba[i, a, j, b] * y1_ba[i, a, j, b]
            ...
            for k in range(nocca):
                for i in range(nocca):
                    for a in range(nvira):
                        for j in range(nocca):
                            for b in range(nvira):
                                c1_a[i, k] += 1.0 * tmp1_aa[i, a, j, b] * y1_aa[k, a, j, b]
                        for j in range(noccb):
                            for b in range(nvirb):
                                c1_a[i, k] += 1.0 * tmp1_ab[i, a, j, b] * y1_ab[k, a, j, b]
            ...
            for k in range(noccb):
                for i in range(noccb):
                    for a in range(nvirb):
                        for j in range(noccb):
                            for b in range(nvira):
                                c1_b[i, k] += 1.0 * tmp1_bb[i, a, j, b] * y1_bb[k, a, j, b]
                        for j in range(nocca):
                            for b in range(nvira):
                                c1_b[i, k] += 1.0 * tmp1_ba[i, a, j, b] * y1_ba[i, a, k, b]
            ...
            for k in range(nocca):
                for i in range(nocca):
                    for p in range(nmoa):
                        c1_a[p, k] -= 1.0 * fock_hfa[i, p] * y1_a[i, k]
            ...
            for k in range(noccb):
                for i in range(noccb):
                    for p in range(nmob):
                        c1_b[p, k] -= 1.0 * fock_hfb[i, p] * y1_b[i, k]
                ...
                for b in range(nvira):
                    for c in range(nvira):
                        ...
                        for i in range(nocca):
                            for a in range(nvira):
                                A = a + nocca
                                c1_a[B, C] += 1.0 * tmp1_aa[i, a, j, b] * y1_aa[i, a, j, c]
                        for i in range(noccb):
                            for a in range(nvirb):
                                A = a + noccb
                                c1_a[B, C] += 1.0 * tmp1_ba[i, a, j, b] * y1_ba[i, a, j, c]
                ...
                for b in range(nvirb):
                    for c in range(nvirb):
                        ...
                        for i in range(noccb):
                            for a in range(nvirb):
                                A = a + noccb
                                c1_b[B, C] += 1.0 * tmp1_bb[i, a, j, b] * y1_bb[i, a, j, c]
                        for i in range(nocca):
                            for a in range(nvira):
                                A = a + nocca
                                c1_b[B, C] += 1.0 * tmp1_ab[i, a, j, b] * y1_ab[i, a, j, c]
                ...
                for i in range(nocca):
                    for a in range(nvira):
                        ...
                        for j in range(nocca):
                            for b in range(nvira):
                                B = b + nocca
                                c1_a[A, C] += 1.0 * tmp1_aa[i, a, j, b] * y1_aa[i, c, j, b]
                        for j in range(noccb):
                            for b in range(nvirb):
                                B = b + noccb
                                c1_a[A, C] += 1.0 * tmp1_ab[i, a, j, b] * y1_ab[i, c, j, b]
                ...
                for i in range(noccb):
                    for a in range(nvirb):
                        ...
                        for j in range(noccb):
                            for b in range(nvirb):
                                B = b + noccb
                                c1_b[A, C] += 1.0 * tmp1_bb[i, a, j, b] * y1_bb[i, c, j, b]
                        for j in range(nocca):
                            for b in range(nvira):
                                B = b + nocca
                                c1_b[A, C] += 1.0 * tmp1_ba[i, a, j, b] * y1_ba[i, c, j, b]
            ...
            for c in range(nvirb):
                for a in range(nvirb):
                    for p in range(nmob):
                        A = a + noccb
                        C = c + noccb
                        c1_b[p, C] -= 1.0 * fock_hfb[A, p] * y1_b[a, c]
