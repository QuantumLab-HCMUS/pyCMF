...
def export_orbitals_to_cube(
    ...
    def _save_cube(tag, data_vec):
        ...
        cubegen.orbital(mol, filepath, data_vec, nx=GRID_SIZE[0], ny=GRID_SIZE[1], nz=GRID_SIZE[2])
...
def calc_tdm(mol, mf_gs, mf_es, svd_threshold=1e-5, gsc_threshold=1e-3):
    ...
    det_a, p_trans_a, mode_a, sa_spectator = calc_transition_density_term(
        s_ao, ca_gs_occ, ca_es_occ, svd_threshold, gsc_threshold
    )
    det_b, p_trans_b, mode_b, sb_spectator = calc_transition_density_term(
        s_ao, cb_gs_occ, cb_es_occ, svd_threshold, gsc_threshold
    )
...
export_settings = {'export_all': False, 'export_range': False, 'export_homo_lumo': False, 'export_specific': False}
